...
if active_module == "⚔️ 模块一：欧亚大盘体系 (包揽标盘让盘的浅中深)":
    ...
    def render_main_handicap_ui(water_level):
        ...
        if st.button(f"执行 {water_level} 单项研判与对冲推演", type="primary", key=f"btn_m_v6_{water_level}"):
            ...
            delta_vals = res_df['动量(Delta)'].values
            ...
            delta_home_std = delta_vals[0]
            delta_home_hcp = delta_vals[3] + delta_vals[4]
            gap_home = round(delta_home_std - delta_home_hcp, 4)
            
            delta_away_std = delta_vals[1] + delta_vals[2]
            delta_away_hcp = delta_vals[5]
...
# ================= 6. 模块三：高级工具 (保持完美运转状态) =================
    ...
    with tab2:
        ...
        if st.button("计算体彩绝对 EV"):
            ...
            st.dataframe(edited_ev.style.applymap(highlight_alerts, subset=['决策定性滤镜']), hide_index=True, use_container_width=True)
