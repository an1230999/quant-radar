...
# ================= 2. 防弹级数学引擎 (启用索引绝对对齐) =================
def calc_pure_prob_series(odds_series):
    """极其安全的归一化函数，返回带原始索引的 Series，彻底杜绝长度不匹配报错"""
...
if active_module == "⚔️ 模块一：欧亚大盘体系 (包揽标盘让盘的浅中深)":
    ...
    def render_main_handicap_ui(water_level):
        ...
        # 强制更换 key 以清空旧缓存
        edited_df = st.data_editor(df, hide_index=True, disabled=cols[3:], use_container_width=True, key=f"main_editor_v4_{water_level}")
        
        if st.button(f"执行 {water_level} 单项研判与对冲推演", type="primary", key=f"btn_m_{water_level}"):
            res_df = edited_df.copy()
            
            # 绝对安全的 Series 索引赋值
            res_df.loc[0:2, '初概率'] = calc_pure_prob_series(res_df.loc[0:2, '初盘'])
            res_df.loc[0:2, '临概率'] = calc_pure_prob_series(res_df.loc[0:2, '临场'])
            res_df.loc[3:5, '初概率'] = calc_pure_prob_series(res_df.loc[3:5, '初盘'])
            res_df.loc[3:5, '临概率'] = calc_pure_prob_series(res_df.loc[3:5, '临场'])
            
            res_df['动量(Delta)'] = (res_df['临概率'] - res_df['初概率']).round(4)
            
            # 七阶热度测算仪
            res_df['七阶热度测算'] = np.where(res_df['动量(Delta)'] >= 0.03, "🌋 极限防范",
                                        np.where(res_df['动量(Delta)'] >= 0.02, "🔥 显著设防",
                                        np.where(res_df['动量(Delta)'] >= 0.008, "🟡 温和流入",
                                        np.where(res_df['动量(Delta)'] <= -0.03, "🧊 极限抛弃",
                                        np.where(res_df['动量(Delta)'] <= -0.02, "📉 显著看衰", 
                                        np.where(res_df['动量(Delta)'] <= -0.008, "↘️ 资金流出", "⚪ 随机噪音"))))))
            
            res_df['理论应有赔率'] = res_df['初盘']
            res_df['净抽水偏离'] = (res_df['临场'] - res_df['理论应有赔率']).round(4)
            
            # 【终极单项研判】：集大成的六维刺探与时空双杀滤镜
            res_df['🎯 终极单项研判 (时空双杀)'] = np.where(
                (res_df['动量(Delta)'] >= 0.015) & (res_df['净抽水偏离'] <= 0), "✅ 黄金共振 (时空双杀，闭眼上)",
            np.where(
                (res_df['动量(Delta)'] <= -0.015) & (res_df['净抽水偏离'] >= 0), "🧊 真实抛弃 (时空双杀，坚决规避)",
            np.where(
                (res_df['动量(Delta)'] >= 0.01) & (res_df['净抽水偏离'] > 0), "☠️ 虚假诱导防范 (升水造热)", 
            np.where(
                (res_df['动量(Delta)'] <= -0.01) & (res_df['净抽水偏离'] < 0), "🕸️ 静态诱网 (深水设伏)", 
            np.where(
                (res_df['动量(Delta)'] >= 0.02) & (res_df['净抽水偏离'] < 0), "🩸 真实暴击防范 (剥削极狠)", 
            np.where(
                (res_df['动量(Delta)'] <= -0.02) & (res_df['净抽水偏离'] > 0), "📈 真实升水抛弃", "⚪ 体系平衡 (伪装平稳)"))))))

            st.dataframe(res_df.style.applymap(highlight_alerts, subset=['七阶热度测算', '🎯 终极单项研判 (时空双杀)']), hide_index=True, use_container_width=True)
            
            # 主客剪刀差研判
            st.markdown("### ⚔️ 欧亚剪刀差极值深度研判 (主客双向解析)")
            return_rate_biao = (1 / (1/res_df.loc[0, '初盘'] + 1/res_df.loc[1, '初盘'] + 1/res_df.loc[2, '初盘'])).round(4)
            
            delta_home_std = res_df.loc[0, '动量(Delta)']
            delta_home_hcp = res_df.loc[3, '动量(Delta)'] + res_df.loc[4, '动量(Delta)']
            gap_home = round(delta_home_std - delta_home_hcp, 4)
            
            delta_away_std = res_df.loc[1, '动量(Delta)'] + res_df.loc[2, '动量(Delta)']
            delta_away_hcp = res_df.loc[5, '动量(Delta)']
...
# ================= 5. 模块二：进球数风控 (安全重构 & 空白T-60兼容) =================
    ...
    def render_goals_ui(water_level):
        ...
        with col_ext1:
            h_handicap = st.number_input(f"【外部变量】主队亚指让球 ({water_level})", value=-0.75, step=0.25, key=f"ext_v4_{water_level}")
        
        st.caption("注：没空看盘时 T-60(J) 列可直接留空！系统将静默 L 列雷达，绝对不影响动量和EV。")
        # 强制更换 key 以清空旧缓存，保证加载完整的 10 行
        edited_df = st.data_editor(df_input, hide_index=True, use_container_width=True, key=f"goal_editor_v4_{water_level}")
        
        if st.button(f"执行 {water_level} 深度风控扫描", type="primary", key=f"btn_g_{water_level}"):
            res_df = edited_df.copy()
            
            # 使用最安全的 Series 索引对齐，不再因为行数报错
            mask_7 = res_df['玩法选项'].str.contains('球') & ~res_df['玩法选项'].isin(['大球', '小球'])
            mask_ou = res_df['玩法选项'].isin(['大球', '小球'])
            
            res_df.loc[mask_7, '初概率'] = calc_pure_prob_series(res_df.loc[mask_7, '初盘(C)'])
            res_df.loc[mask_7, 'J概率'] = calc_pure_prob_series(res_df.loc[mask_7, 'T-60(J)'])
            res_df.loc[mask_7, '临概率'] = calc_pure_prob_series(res_df.loc[mask_7, '临场(D)'])
            
            res_df.loc[mask_ou, '初概率'] = calc_pure_prob_series(res_df.loc[mask_ou, '初盘(C)'])
            res_df.loc[mask_ou, 'J概率'] = calc_pure_prob_series(res_df.loc[mask_ou, 'T-60(J)'])
            res_df.loc[mask_ou, '临概率'] = calc_pure_prob_series(res_df.loc[mask_ou, '临场(D)'])
            
            res_df['总动量(Delta)'] = (res_df['临概率'] - res_df['初概率']).round(4)
            res_df['期望值(EV)'] = (res_df['初概率'] * res_df['临场(D)'] - 1).round(4)
            res_df['加速度(V-Delta)'] = (res_df['临概率'] - res_df['J概率']).round(4)
            
            res_df = generate_goal_radars(res_df, z2, z3, v_limit)
            
            st.dataframe(res_df[['玩法选项', '初盘(C)', '临场(D)', '总动量(Delta)', '期望值(EV)', '加速度(V-Delta)', '动量雷达 (G列)', 'EV价值仪 (H列)', '防伪验证 (I列)', '主力狙击 (L列)']].style.applymap(highlight_alerts, subset=['动量雷达 (G列)', 'EV价值仪 (H列)', '防伪验证 (I列)', '主力狙击 (L列)']), hide_index=True, use_container_width=True)
            
            st.markdown("### 📐 静态底座 X 光透视分析")
            if mask_7.sum() > 0 and not res_df.loc[mask_7, '初概率'].isna().all():
                try:
                    even_prob = round(res_df.loc[mask_7, '初概率'].iloc[[0,2,4,6]].sum(), 4)
                    odd_prob = round(res_df.loc[mask_7, '初概率'].iloc[[1,3,5,7]].sum(), 4)
                    ...
                    static_min_odds_index = res_df.loc[mask_7, '初盘(C)'].idxmin()
                    static_highest_prob_goal = res_df.loc[static_min_odds_index, '玩法选项']
