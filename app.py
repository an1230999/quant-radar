...
# ==============================================================================
# ================= 🛡️ 核心大坝：跨维交割影子数据扶正拦截器 ========================
# ==============================================================================
# 当且仅当用户切离当前的模块二组合时，才将无冲突影子备份数据洗回常驻输入源，完美兼顾不闪退与不丢失！
if "last_active_m2_match" in st.session_state and "last_active_m2_wl" in st.session_state:
    l_match = st.session_state["last_active_m2_match"]
    l_wl = st.session_state["last_active_m2_wl"]
    shadow_k = f"m2_shadow_{l_match}_{l_wl}"
    target_k = f"m2_base_df_{l_match}_{l_wl}"
    if shadow_k in st.session_state:
        st.session_state[target_k] = st.session_state[shadow_k]
...
if active_module == "🎯 模块七：全息连通器·深盘猎杀终端 (V30)":
    ...
    if st.session_state[calc_key_m7]:
        ...
        try:
            ...
            # ========================================================
            # 幽灵标盘逆向重构 (Phantom 1X2 Synthesizer)
            # ========================================================
            if is_all_std_closed:
                if K_int < 0: 
...
# ==============================================================================
    ...
    def render_module_x_ui(wl, match_id):
        ...
        st.write("")
        if st.button("🚀 启动大盘底座联动扫描", key=f"btn_{calc_key}"):
        ...
        if st.session_state[calc_key]:
            ...
            if xg_h_m3 < 0 or xg_a_m3 < 0: st.error("⚠️ 预期进球为负，请检查设置！")
                ...
                with tab_m3_2:
                    ...
                    ev_vals = np.round(d_odds * int l_prob - 1, 4)
    ...
    * 🧮 **【看板二 (总球数防假账)】**：对比球数和比分积分。**注意：如果你没有在比分台里输入对应进球数（比如0球）的比分（如0-0），看板二必然会因为没有数据而报警 `🚨 总池空壳抽水`。** 只有当对应进球数下的主要比分都输全了，这里的报警才是真正的庄家抽水铁证！
    ...
    if st.session_state[calc_key_m2]:
        try:
            ...
            st.dataframe(df_goals_check := pd.DataFrame(goal_checks), hide_index=True, use_container_width=True)
