...
# ================= 1. 全局配置与UI优化 =================
st.set_page_config(page_title="FX2 量化对冲终端 [终极重构至尊版]", layout="wide", page_icon="🏦")
...
# 🛡️ [安全升级] Dixon-Coles 引入高期望指数阻尼，防止全局矩阵负概率坍塌
def dixon_coles_full_matrix(lambda_, mu_, rho_, is_knockout=False):
    ...
    max_xg = max(lambda_, mu_)
    damping = math.exp(-(max_xg - 2.5)) if max_xg > 2.5 else 1.0
    rho_adj = rho_ * damping

    max_calc = 15 
    px, py = poisson_pmf_array(lambda_, max_calc), poisson_pmf_array(mu_, max_calc)
    P = np.outer(px, py)
    P[0, 0] *= (1 - lambda_ * mu_ * rho_adj)
    P[1, 0] *= (1 + lambda_ * rho_adj)
    P[0, 1] *= (1 + mu_ * rho_adj)
    P[1, 1] *= (1 - rho_adj)
    P = np.clip(P, 0, 1)
...
# 🛡️ [物理无损] 四分盘隐式解耦，无视 int() 导致的数据坍塌
def calc_quarter_handicap_poisson(xg_h, xg_a, rho, k_val):
    _, _, _, _, _, P_mat = dixon_coles_full_matrix(xg_h, xg_a, rho)
    def get_slice_prob(k):
        pw = sum(P_mat[i, j] for i in range(8) for j in range(8) if i - j > -k)
        pd = sum(P_mat[i, j] for i in range(8) for j in range(8) if i - j == -k)
        pl = sum(P_mat[i, j] for i in range(8) for j in range(8) if i - j < -k)
        return np.array([pw, pd, pl])

    rem = abs(k_val) % 0.5
    if abs(rem - 0.25) < 0.001:
        k1 = k_val + 0.25 if k_val > 0 else k_val - 0.25
        k2 = k_val - 0.25 if k_val > 0 else k_val + 0.25
        return np.round(0.5 * get_slice_prob(k1) + 0.5 * get_slice_prob(k2), 4)
    else:
        return np.round(get_slice_prob(k_val), 4)

# 🛡️ 跨盘守恒的动态残差边界
def calc_dynamic_residual_threshold(base_res, k_val, xg_val):
    k_depth = max(0.0, abs(k_val) - 1.0)
    xg_factor = 1.0 / math.sqrt(max(0.1, xg_val))
    return round(base_res * (1.0 + 0.15 * k_depth) * xg_factor, 4)

# 🛡️ 标亚向量分歧指数 Div
def calc_divergence_index(line_shift, flow_1x2):
    if abs(line_shift) < 0.001 or abs(flow_1x2) < 0.001: return 0.0
    return round(np.sign(line_shift) * np.sign(flow_1x2) * abs(line_shift * flow_1x2), 4)

def check_high_tension_equilibrium(p_init_h, p_init_a, k_val):
    if abs(p_init_h - p_init_a) <= 0.0800 and abs(k_val) <= 0.25:
        return True, "⚡ 【高张力均势场】底座识别为强强碰撞/保守场域，已自动静默大胜穿盘，提升一球小胜与冷平1-1权重。"
    return False, ""
...
if active_module == "🪐 模块八：M8 欧亚双轨全息对撞舱 (1X2+AH)":
    ...
    st.caption("【专业机构级】透视平博与365资金差，综合标盘、亚指位移、亚指水位三维齿轮咬合。熔铸 [返还率交叉审讯] 与 [标亚分歧指数]！")
    ...
    if is_home_up: st.info(f"🔵 **系统广播：识别为主队让球。本场判定：【{up_name} = 上盘】，【{down_name} = 下盘】。**")
    else: st.warning(f"🟡 **系统广播：识别为主队受让。本场判定：【{up_name} = 上盘】，【{down_name} = 下盘】。**")
    ...
    with col_L:
        ...
        with c_la1:
            eu_ah_line_c = safe_number_input("初盘盘口", f"eu_ah_line_c_{current_match}", m8_init_line, step=0.25)
            eu_ah_line_d = safe_number_input("临场盘口", f"eu_ah_line_d_{current_match}", -0.75, step=0.25)
        with c_la2:
            eu_ah_up_c = safe_number_input("初盘上盘水", f"eu_ah_up_c_{current_match}", 1.95)
            eu_ah_up_d = safe_number_input("临场上盘水", f"eu_ah_up_d_{current_match}", 2.05)
        with c_la3:
            eu_ah_down_c = safe_number_input("初盘下盘水", f"eu_ah_down_c_{current_match}", 1.95)
            eu_ah_down_d = safe_number_input("临场下盘水", f"eu_ah_down_d_{current_match}", 1.85)
    ...
    with col_R:
        ...
        with c_ra2:
            ret_ah_up_c = safe_number_input("初盘上水", f"ret_ah_up_c_{current_match}", 1.90)
            ret_ah_up_d = safe_number_input("临场上水", f"ret_ah_up_d_{current_match}", 2.00)
        with c_ra3:
            ret_ah_down_c = safe_number_input("初盘下水", f"ret_ah_down_c_{current_match}", 1.90)
            ret_ah_down_d = safe_number_input("临场下水", f"ret_ah_down_d_{current_match}", 1.80)
    ...
    if st.button("🚀 启动全白盒欧亚多维测谎", type="primary", use_container_width=True):
        ...
        try:
            ...
            eu_line_shift = eu_ah_line_d - eu_ah_line_c
            ret_line_shift = ret_ah_line_d - ret_ah_line_c
            
            eu_payout_c = 1.0 / np.sum(1.0 / np.array([eu_1x2_w_c, eu_1x2_d_c, eu_1x2_l_c]))
            eu_payout_d = 1.0 / np.sum(1.0 / np.array([eu_1x2_w_d, eu_1x2_d_d, eu_1x2_l_d]))
            margin_shift = round(eu_payout_d - eu_payout_c, 4)
            
            main_w_flow = eu_w_flow if is_home_up else eu_l_flow 
            main_ret_w_flow = ret_w_flow if is_home_up else ret_l_flow
            div_index = calc_divergence_index(eu_line_shift, main_w_flow)

            def describe_flow_tag(flow):
                return f"🔴 大幅流入 (+{flow*100:.1f}%)" if flow > 0.02 else f"📈 缓慢流入 (+{flow*100:.1f}%)" if flow > 0.005 else f"🧊 坚决流出 ({flow*100:.1f}%)" if flow < -0.02 else f"📉 缓慢流出 ({flow*100:.1f}%)" if flow < -0.005 else f"⚪ 平稳换手 ({flow*100:+.1f}%)"
            def describe_line_tag(shift):
                return f"🔺 强硬升盘 ({shift:+.2f})" if shift < 0 else f"🔻 退盘诱导 ({shift:+.2f})" if shift > 0 else "➖ 盘口未动"
                
            st.markdown("### 🔬 第三步：独立机构微观切片与几何物理诊断")
            c_rpt1, c_rpt2 = st.columns(2)
            with c_rpt1:
                st.markdown("#### 🛡️ 平博(左轨) - 真实意图侦测")
                st.write(f"• **主胜 ({describe_flow_tag(eu_w_flow)})** | **平局 ({describe_flow_tag(eu_d_flow)})** | **客负 ({describe_flow_tag(eu_l_flow)})**")
                st.write(f"• **亚盘动量**：位移 **{describe_line_tag(eu_line_shift)}** | 上盘水 **{describe_flow_tag(eu_up_flow)}**")
                st.info(f"💡 **[Patch 04 标亚向量分歧] Div = {div_index:+.4f}** | 返还率变动: **{margin_shift*100:+.2f}%**")
            with c_rpt2:
                st.markdown("#### 🛒 365(右轨) - 散户情绪雷达")
                st.write(f"• **主胜 ({describe_flow_tag(ret_w_flow)})** | **平局 ({describe_flow_tag(ret_d_flow)})** | **客负 ({describe_flow_tag(ret_l_flow)})**")
                st.write(f"• **亚盘动量**：位移 **{describe_line_tag(ret_line_shift)}** | 上盘水 **{describe_flow_tag(ret_up_flow)}**")
                
            st.markdown("---")
            st.markdown("### ⚔️ 第四步：跨机构认知剪刀差对撞台")
            df_diff = pd.DataFrame([
                {"监控项": "上盘标盘流速", "平博(Smart)": f"{main_w_flow:+.4f}", "365(Retail)": f"{main_ret_w_flow:+.4f}", "认知剪刀差": f"{main_w_flow - main_ret_w_flow:+.4f}"},
                {"监控项": "亚指位移", "平博(Smart)": describe_line_tag(eu_line_shift), "365(Retail)": describe_line_tag(ret_line_shift), "认知剪刀差": "➖"},
                {"监控项": "上盘水位流速", "平博(Smart)": f"{eu_up_flow:+.4f}", "365(Retail)": f"{ret_up_flow:+.4f}", "认知剪刀差": f"{eu_up_flow - ret_up_flow:+.4f}"},
            ])
            st.dataframe(df_diff, hide_index=True, use_container_width=True)

            st.markdown("---")
            st.markdown("### 🚀 第五步：欧亚三维交叉定性雷达 (结合返还率与分歧审判)")
            verdict_box = st.container()
            
            if div_index > 0:
                if main_w_flow > 0 and eu_line_shift > 0:
                    verdict_box.error("### 🚨 【降档吸筹 / 震荡控赔诱下局】 (Div > 0 向量撕裂)\n\n"
                                      f"**【分歧侦测】**：亚盘退盘放宽门槛吸引下盘，但标盘上盘胜率疯狂降水吸筹 (+{main_w_flow*100:.1f}%)！\n"
                                      "**【🎯 终极裁决】**：庄家故意消耗散户耐心，暗中低成本吸筹。主队极易刚好赢一球卡盘！")
                elif main_w_flow < 0 and eu_line_shift < 0:
                    verdict_box.error("### 🚨 【空头拉高 / 升盘虚热诱上局】 (Div > 0 向量撕裂)\n\n"
                                      f"**【分歧侦测】**：亚盘强行加深门槛假装强悍，标盘最敏感主胜率却在升水出逃 ({main_w_flow*100:.1f}%)！\n"
                                      "**【🎯 终极裁决】**：虚火造热，引诱买入上盘送死，坚决去下盘避险！")
            elif main_w_flow > 0.015 and (eu_line_shift < 0 or (eu_line_shift == 0 and eu_up_flow > 0.015)):
                if margin_shift >= 0: verdict_box.success("### 🚀 【真龙现身 / 欧亚全息量子共振】\n\n降低赔付的同时给市场让利，且防线严密。实力碾压，无脑重锤上盘！")
                else: verdict_box.success("### 🚀 【真龙现身 / 常规物理设防】\n\n多空顺流，庄家升盘缩水常规防守。顺大势冲上盘！")
            elif main_w_flow > 0.015 and (eu_line_shift > 0 or (eu_line_shift == 0 and eu_up_flow < -0.015)):
                verdict_box.error("### 🚨 【声东击西 / 经典赢球输盘陷阱】\n\n标盘降水诱导‘稳赢’，决定赢几个球的亚盘却悄悄降低门槛放水！极大概率最多赢一球卡盘！")
            elif main_w_flow < -0.01 and main_ret_w_flow > 0.01:
                verdict_box.error("### 🩸 【跨轨绞杀 / 散户屠宰场】\n\n全球聪明钱抛弃上盘，零售机构强压赔率诱导散户接盘！极度危险，立刻逃离上盘！")
            else:
                verdict_box.markdown("### ⚖️ 【多空焦灼 / 常规物理博弈】\n\n三维齿轮未出现明显偏离，平博与365步调一致，建议结合 M10/M9 深度辅助判定。")
...
# ==============================================================================
    ...
    with c10_1: m10_ah = safe_number_input("机构亚指盘口 (主让为负)", f"m10_ah_{current_match}", -0.5, format="%.2f", step=0.25)
    with c10_2:
        st.info("💡 提示：深盘局开启【外推代偿】后，将自动静默针对浅盘的专属冷门警报器。")
        m10_unlock = st.toggle("🔓 开启深盘外推代偿模式 (强制越狱浅盘结界)", value=True)
    ...
    def create_m10_input_grid(period_target, match_id):
        ...
        with col_p1: act_p = safe_number_input(f"实际查到的场次数", f"m10_act_p_{period_target}_{match_id}", period_target, format="%.0f", step=1.0)
        with col_p2: st.info(f"💡 提示：如球队未打满 {period_target} 场，请如实修改场次，底层会自动折算！")
        act_p = max(act_p, 1.0) 
        ...
        f = {}
        f['diff_venue_g'] = (h_h_g / act_p) - (a_a_g / act_p)
        f['diff_venue_c'] = (h_h_c / act_p) - (a_a_c / act_p)
        f['spear_h'] = (h_h_g / act_p) - (a_a_c / act_p)
    ...
    with t10_3: 
        ...
        if use_imputation:
            st.success("✅ 已开启代偿：底层引擎已自动克隆【近 10 场】均值特征。")
            feats_15 = feats_10.copy()
        else: feats_15 = create_m10_input_grid(15, current_match)
    ...
    if st.button("🚀 呼叫斯巴达量化军团 (启动 Meta-Model 预测)", type="primary", use_container_width=True):
        ...
        if is_deep_pan and not m10_unlock:
            st.error(f"⛔ **【盘口熔断触发】**：本场亚指为 {m10_ah}，属深盘局！超出浅盘训练能力圈，请开启上方【🔓 深盘外推代偿】进行强制越狱！")
        else:
            comp_val = (abs(m10_ah) - 1.0) * 0.16 if is_deep_pan else 0.0
            if is_deep_pan: st.warning(f"⚠️ **【深盘越狱已激活】**：自动静默冷门警报，向强势方强行注入 {comp_val*100:.1f}% 物理代偿势能！")
            ...
            st.markdown("#### 🚨 斯巴达冷门猎手 拦截器")
            upset_alert = False
            if not is_deep_pan:
                if m10_ah <= -0.25 and consensus_h_50 < 0.35 and feats_10['diff_venue_g'] < 0.5:
                    st.error(f"🩸 **【深水爆冷绝杀警报】** 主队强行让球，但主胜共识度极低 ({consensus_h_50:.0%})，历史爆冷率 68%！")
                    upset_alert = True
                if m10_ah >= 0.25 and feats_10['spear_h'] > 1.2:
                    st.error(f"🩸 **【反客为主诱导警报】** 强开客让，但主队主场长矛锋利 (差值 {feats_10['spear_h']:.2f})！主队极大概率不败！")
                    upset_alert = True
                if not upset_alert: st.info("⚪ 未触发致命级爆冷规则，属常规多空博弈局。")
            else: st.info("🤫 深盘越狱模式开启中，已自动强制静默浅盘专属冷门规则。")

            st.markdown("---")
            st.markdown("#### 🎖️ Meta-Model 高级指挥官最终裁决")
            def get_std_desc(p): return "⭐⭐⭐ 绝对碾压" if p>=0.65 else "⭐⭐ 具备核心优势" if p>=0.50 else "⚪ 势均力敌区" if p>=0.35 else "🧊 实力极度疲软"
            def get_fail_desc(p, act): return "➖ 盘口未激活" if not act else "🩸 【高危】大概率输盘" if p>=0.60 else "🟡 【裂痕】实力摇摆" if p>=0.45 else "🛡️ 【稳固】穿盘充沛"

            df_4d = pd.DataFrame([
                {"指挥官职能": "1号官：主胜(正路)", "独立概率": f"{prob_hw*100:.1f}%", "极客暗语": get_std_desc(prob_hw)},
                {"指挥官职能": "2号官：客胜(逆路)", "独立概率": f"{prob_aw*100:.1f}%", "极客暗语": get_std_desc(prob_aw)},
            ...
            ah_verdict = ""
            if m10_ah < 0:
                c_ah1.metric("当前亚指", f"{m10_ah}")
                ah_verdict = "✅ **【上盘稳健】(主队穿盘)**：实力压制且防线稳固，顺路重锤上盘！" if prob_hw>0.60 and prob_hf<0.40 else "🚨 **【赢球输盘】(去下盘)**：主胜极热但输盘概率居高不下，盘口过深，极易一球小胜卡盘！" if prob_hw>0.60 and prob_hf>=0.50 else "🧊 **【全面崩塌】(直接去下盘)**：让球但实力根本不足以赢球，必死无疑。" if prob_hw<0.50 and prob_hf>0.60 else "⚖️ **【盘口平衡】**：物理基本面与让球数严丝合缝。"
            elif m10_ah > 0:
                c_ah1.metric("当前亚指", f"+{m10_ah}")
                ah_verdict = "✅ **【上盘稳健】(客队穿盘)**：客胜绝对碾压，阻力微弱。" if prob_aw>0.60 and prob_af<0.40 else "🚨 **【客队赢球输盘】(去主队受让)**：客热但受困深盘，极容易一球小胜卡盘。" if prob_aw>0.60 and prob_af>=0.50 else "🧊 **【客队崩塌】(去主队受让)**：强开客让纯属诱导，主队可保不败。" if prob_aw<0.50 and prob_af>0.60 else "⚖️ **【盘口平衡】**：无明显物理套利空间。"
            else:
                c_ah1.metric("当前亚指", "平手 (0)")
                ah_verdict = "✅ **【主队优势】** 平手盘下主队至少不败。" if prob_hw>prob_aw+0.15 else "✅ **【客队优势】** 直接拿客队。" if prob_aw>prob_hw+0.15 else "⚖️ **【绝对均势】** 真正实力五五开局，大平局首选！"
...
# ==============================================================================
    ...
    st.caption("【微创手术终局】专杀竞彩定向卷。提取任意K值深盘连通桥，一字不差逆向重构幽灵标盘1X2，熔铸 [自适应物理残差边界]！")
    ...
    with col_std:
        if is_all_std_closed: st.warning("🔒 官方整盘屏蔽标盘，由泊松代偿引擎在后台逆向解耦。")
    ...
    if st.session_state[calc_key_m7]:
        ...
        try:
            ...
            dyn_thresh = calc_dynamic_residual_threshold(0.0080, m7_k, m7_tg)
            st.info(f"💡 **[Patch 02 自适应残差边界]** 依据让球深度与xG期望，本场连通器物理排雷界限锁死于：± **{dyn_thresh:.4f}**")
            ...
            for i in range(6):
                if i < 3 and is_all_std_closed:
                    intra.append("🔒 锁盘"); lie_r_show.append("➖"); rmv_show.append("➖"); verdicts.append("🚫 官方未售"); scripts.append("底层已自动代入泊松物理纯率作为镜像支点。")
                    continue

                flow, res, r = d_all[i], residuals[i], rmv[i]
                intra.append("🔥 主力真金狂买" if flow > 0.025 else "🕳️ 筹码夺路出逃" if flow < -0.025 else "⚪ 散户微幅换手")
                lie_r_show.append(f"{res:+.4f} (🔴虚高造热)" if res > dyn_thresh else f"{res:+.4f} (🟢真实筑墙)" if res < -dyn_thresh else f"{res:+.4f} (⚪合理容差)")
                rmv_show.append(f"{r*100:+.2f}% (🔴致命诱导)" if r > 0.04 else f"{r*100:+.2f}% (🟢绝对核心)" if r < -0.04 else f"{r*100:+.2f}% (⚪常规波动)")

                is_lie, is_gold = (res > dyn_thresh and r > 0.04), (res < -dyn_thresh and r < -0.04)
                if is_lie: verdicts.append("🚨 镜像畸高 (造热死坑)"); scripts.append("跨盘概率被虚假拔高，克扣赔率制造稳赢假象，坚决排除。")
                elif is_gold:
                    if flow > -0.0100: verdicts.append("💎 全息闭环暗水王"); scripts.append("承接对冲纯率！机构承受最真实的铁壁，全场第一位！")
                    else: verdicts.append("🧊 镜像被弃死冷"); scripts.append("传动链与市场流速同步宣判死刑，冷门通道被封焊。")
                else:
                    if flow >= 0.0250: verdicts.append("✅ 明牌顺势御流位"); scripts.append("主力真金狂买，顺牌冲！")
                    elif flow <= -0.0250: verdicts.append("⏬ 顺流全息抛弃位"); scripts.append("同步放弃此端，呈出逃态势。")
                    elif abs(res) > dyn_thresh * 0.5: verdicts.append("🟡 盘面轻微形变"); scripts.append("存在微弱受力偏移，建议结合大盘轨迹。")
                    else: verdicts.append("⚪ 连通器支点平衡"); scripts.append("常规受力过渡位，多空动态平衡。")

            st.markdown("### 📊 V30 幽灵重构·微创大终局体检表")
            df_out_m7 = pd.DataFrame({"投注选项": opts_std + opts_let, "临场纯率": pd_show_list, "流速动能(一阶)": intra, "连通器残差": lie_r_show, "变异度(RMV)": rmv_show, "传动时空裁决": verdicts, "精算审讯结论": scripts})
...
# ==============================================================================
    ...
    st.caption("【终极合并工作台】整合底座、DC期望与异构敞口。全面熔铸 [全局防溢出]、[四分盘解耦]、[自适应残差]、[非对称暗水] 等硬核物理网！")
    ...
    def render_module_x_ui(wl, match_id):
        ...
        with c1: mx_tg = safe_number_input("大小球期望", f"mx_tg_{match_id}_{wl}", 2.75, format="%.2f", step=0.25)
        ...
        with c4: mx_k = safe_number_input("体彩让球数(K)", f"mx_k_{match_id}_{wl}", -1.0, format="%.2f", step=0.25) 
        with c5: mx_rho = safe_number_input("DC依赖系数", f"mx_rho_{match_id}_{wl}", -0.15, format="%.2f", step=0.01)
        ...
        if st.session_state[calc_key]:
            ...
            # 🛡️ 启用四分盘隐式解耦防坍塌计算
            p_let_slice = calc_quarter_handicap_poisson(xg_h_m3, xg_a_m3, mx_rho, mx_k)
            p_let_w, p_let_d, p_let_l = p_let_slice[0], p_let_slice[1], p_let_slice[2]
            
            s_theo = np.round([p_std_w, p_std_d, p_std_l, p_let_w, p_let_d, p_let_l], 4)
            u_theo = s_theo.copy()

            is_high_tension, tension_msg = check_high_tension_equilibrium(biao_c[0], biao_c[2], mx_k)
            if is_high_tension: st.warning(tension_msg)

            ret_rate_c_std = np.nansum(1.0 / c_odds[0:3])
            ret_rate_d_std = np.nansum(1.0 / d_odds[0:3])
            margin_shift = round(ret_rate_d_std - ret_rate_c_std, 4)
            
            vol = np.nanstd(delta)
            dyn_z3 = min(max(round(vol * 0.8, 4), 0.0040), 0.0150)
            dyn_z4 = min(max(round(vol * 1.2, 4), 0.0060), 0.0200)
            st.info(f"📊 动态流速阈值：显著流込 Z3={dyn_z3:.4f} | 核心结构 Z4={dyn_z4:.4f} | 返还率变动: **{margin_shift*100:+.2f}%**")
            ...
            for i in range(6):
                ...
                if not pd.isna(s_t) and not pd.isna(u_t) and not pd.isna(c_prob):
                    ...
                    if delta[i] >= dyn_z3: 
                        ...
                        if is_dominant: aa_hedge[i] = "🎁 暗度陈仓(核心轴)" if struct >= dyn_z4 else "🧊 极限绞杀(基底核心)" if struct <= -dyn_z4 else "⚪ 主流流出"
                        else: aa_hedge[i] = "⚪ 泄洪波及(非主线)"
                    else: 
                        if struct >= dyn_z3: aa_hedge[i] = "🌋 静态死防"
            ...
            # 🛡️ 跨盘守恒自适应残差红线 (带平手盘短路保护)
            res_std_w, res_let_l, max_residual = 0.0, 0.0, 0.0
            if abs(mx_k) >= 0.25:
                res_std_w = prob_d[0] - (prob_d[3] + prob_d[4]) if abs(mx_k + 1.0) < 0.25 else 0
                res_let_l = prob_d[5] - (prob_d[1] + prob_d[2]) if abs(mx_k + 1.0) < 0.25 else 0
                max_residual = max(abs(res_std_w), abs(res_let_l))
                
            dyn_res_threshold = calc_dynamic_residual_threshold(0.0050, mx_k, mx_tg)
            ...
            if is_home_fake_heat:
                intent = "虚假大热局 (低赔诱导)"
                warning = f"⚠️ **【危险预警：主队虚假大热】** 标盘主胜降水造热(动量{delta[0]:+.4f})，但底座胜率仅{s_theo[0]*100:.1f}%！底层让负升水掩护，坚决规避主胜！"
            elif is_away_fake_heat:
                intent = "虚假大热局 (低赔诱导)"
                warning = f"⚠️ **【危险预警：客队虚假大热】** 标盘客胜降水造热(动量{delta[2]:+.4f})，但底座胜率仅{s_theo[2]*100:.1f}%！底层让胜升水掩护，坚决规避客胜！"
            elif max_raise_val > 0.035 and max_residual < dyn_res_threshold and margin_shift >= 0:
                intent = "量子纠缠态 (机构通杀局)"
                warning = f"🚨 **【极危警告：量子通杀】** 【{opt_raise_name}】极限升水，跨盘残差仅 {max_residual:.4f} (<{dyn_res_threshold:.4f})，且返还率上升！放弃本场或极小注博冷！"
            elif margin_shift < -0.0200:
                intent = "资金平衡局 (机构不确定)"
                warning = "⚠️ 机构大幅缩表降抽水！说明对赛果无绝对把握，依靠提高抽水对冲，降水防范逻辑可能失效。"
            elif max_residual > dyn_res_threshold * 2.5 and positive_flows >= 3:
                intent = "资金失衡局 (散户倒逼)"
                warning = "⚠️ 跨盘口守恒律严重撕裂！散户倒逼机构被动调盘，'大热必死'概率极高。"
            elif positive_flows <= 2 and max_drop_val > 0.015 and max_residual < dyn_res_threshold:
                intent = "信息优势局 (机构确知)"
                warning = f"💎 针对【{opt_drop_name}】极限降水，且跨盘残差极小 (<{dyn_res_threshold:.4f})。机构主动设防，置信度最高！"
            else:
                warning = "⚠️ 常规物理博弈。重点参考主流动量与极高潜 Skew 选项。"
                intent = "常规换手局"

            st.markdown("---")
            st.markdown("#### 🕵️ 机构意图分类器 & 守恒残差审讯")
            if intent in ["信息优势局 (机构确知)", "常规换手局"]: st.success(warning)
            elif intent in ["量子纠缠态 (机构通杀局)", "虚假大热局 (低赔诱导)"]: st.error(warning)
            else: st.warning(warning)
            st.info(f"💡 判定结论：**{intent}** | 跨盘残差: **{max_residual:.4f}** (动态红线: **{dyn_res_threshold:.4f}**) | 返还率差: **{margin_shift*100:+.2f}%**")

            def cross_six_diagnosis(d_w, d_d, d_l, d_lw, d_ld, d_ll, K):
                if is_high_tension: return "⚡ 【均势高张力场】初盘极度胶着，多空往复频繁。优先盯防一球小胜卡盘或 1-1 冷平点位！"
                cond1 = (d_w < -0.01 and d_lw < -0.01 and abs(d_ld) < 0.003 and d_d > 0.01 and d_ll > 0.01 and abs(d_l) < 0.003)
                if cond1 and abs(K + 1.0) < 0.25: return "🚨 【一球小胜铁幕】标胜与让胜双降，让平死守，平局与让负拉高！严防主队刚好赢1球 (1-0, 2-1)。"
                if (d_w < -0.015 and d_lw > 0 and d_ld > 0): return "⚠️ 【标盘虚空造热】标胜降水，底层让胜让平却升水！标盘强造主胜吸散户，真金未入让盘。防冷门！"
                if (d_w > 0.02 and d_d > 0.02 and d_l < -0.015 and d_ll < -0.015): return "🩸 【主队全面崩塌】标胜标平同步被抛，资金涌入客队不败。直接去下盘！"
                if (d_d > 0.015 and d_ld < 0.005 and d_ll > 0.01): return "🎯 【平局/客不败双轨异动】标平概率暴增，让平未跟随，让负大幅流入！防冷平或客胜！"
                if max_residual > dyn_res_threshold: return f"🛡️ 【跨盘守恒律撕裂】存在强烈的非对称对冲行为，结合 M3 底座寻找价值盲区。"
                return "⚪ 常规物理博弈，6项流速基本同步。"

            st.markdown("#### 🧬 6项联合交叉诊断 (终极剧本破译)")
            final_verdict = cross_six_diagnosis(delta[0], delta[1], delta[2], delta[3], delta[4], delta[5], mx_k)
            st.error(f"🎯 终极裁决：{final_verdict}")

            st.markdown("---")
            st.markdown("## 🎫 第二维：DC双泊松高阶价值提纯")
            df_m, ph2, ph1, pdr, pau, _ = dixon_coles_full_matrix(xg_h_m3, xg_a_m3, mx_rho)
            tab_m3_1, tab_m3_2 = st.tabs(["🧮 DC 进球矩阵", "✂️ 体彩 Skew 切片器"])
            with tab_m3_1:
            ...
            with tab_m3_2:
                ...
                # 体彩被动抽水剥离
                margin_std = np.nansum(1.0 / d_odds[0:3]) - 1 if not pd.isna(d_odds[0:3]).all() else 0.11
                margin_let = np.nansum(1.0 / d_odds[3:6]) - 1 if not pd.isna(d_odds[3:6]).all() else 0.11
                margins = np.array([margin_std]*3 + [margin_let]*3)
                
                # 局部物理栅栏：V-Skew 波动调整与相对价值偏度
                skew_vals = np.round((d_odds * intl_prob) - (1.0 + margins), 4)
                v_skew = np.where(skew_vals > 0, skew_vals * min(1.0, 1.0 / math.sqrt(max(0.1, mx_tg))), skew_vals)
                
                judge_m3 = np.full(6, "➖", dtype=object)
                for i in range(6):
                    if pd.isna(skew_vals[i]): continue
                    if v_skew[i] > 0.015 and delta[i] > 0: judge_m3[i] = "🌟 绝对正向盲区"
                    elif skew_vals[i] > -0.06 and delta[i] > 0.025 and (intl_prob[i] * d_odds[i] > 1.0): judge_m3[i] = "🔥 动量强行填平"
                    elif v_skew[i] > 0 and delta[i] < -0.015: judge_m3[i] = "🕳️ 左侧价值陷阱"
                    elif intl_prob[i] > 0.65 and delta[i] >= -0.005: judge_m3[i] = "🛡️ 刚性物理铁壁"
                    else: judge_m3[i] = "🟢 极度高潜" if skew_vals[i] >= -0.02 else "🟡 合理磨损" if skew_vals[i] >= -0.06 else "📉 劣势赔付" if skew_vals[i] >= -0.10 else "🩸 抽水深渊"
                
                out_df3 = pd.DataFrame({"投注项": ["标准胜", "标准平", "标准负", "让球胜", "让球平", "让球负"], "推演纯率": np.round(intl_prob, 4), "相对偏度 Skew": skew_vals, "雷达定性": judge_m3})
                st.dataframe(out_df3.fillna(""), hide_index=True, use_container_width=True)
                st.info("💡 **[提纯保护网已激活]** 已剥离高额抽水，并熔铸 $V\_Skew$ 与 $\Delta Flow$ 2D 正交过滤，无伤排雷。")
            ...
            with tab_m4_a:
                ...
                mismatch_adj = round(xg_diff - (-mx_hcp_bookie) * (1.0 + 0.15 * abs(mx_hcp_bookie)), 4)
                
                c1_m4, c2_m4, c3_m4 = st.columns(3)
                c1_m4.metric(f"机构物理开盘", f"{mx_hcp_bookie}")
                c2_m4.metric("泊松推演净胜", f"{xg_diff}")
                c3_m4.metric("🌪️ 非线性时空撕裂度", f"{mismatch_adj}")
                
                if mismatch_adj >= 0.35: st.success("✅ **物理价值深凹局：** 真实穿盘防御，主队大概率穿盘！")
                elif mismatch_adj <= -0.35: st.error("🚨 **空头让步假象：** 极致诱杀陷阱，坚决去下盘/客队不败！")
                else: st.warning("⚖️ **盘理物理平衡：** 结构严丝合缝。")
            ...
            with tab_m4_b:
                ...
                if np.isnan(d_odds_m4).any() or (d_odds_m4 <= 0).any(): st.warning("⚠️ 标盘数据缺失。")
                    ...
                    # 🛡️ [物理无损] 引入指数级防穿透边界 (exp) 的非对称凯利敞口
                    delta_w = delta[0] if not pd.isna(delta[0]) else 0
                    liability_m4 = np.zeros(3)
                    liability_m4[0] = fair_prob_m4[0] * d_odds_m4[0] * math.exp(delta_w * 5.0)
                    liability_m4[1] = fair_prob_m4[1] * d_odds_m4[1]
                    liability_m4[2] = fair_prob_m4[2] * d_odds_m4[2] * math.exp(-delta_w * 2.5)
                    
                    df_kelly = pd.DataFrame({"赛果": ["主胜", "平局", "客胜"], f"临场赔率": d_odds_m4, "真实公平概率": np.round(fair_prob_m4, 4), "⚠️ 非对称敞口指数": np.round(liability_m4, 4)})
                    st.dataframe(df_kelly, hide_index=True, use_container_width=True)
                    max_idx = int(np.argmax(liability_m4))
                    st.error(f"💣 **暗水敞口警报：** 融合资金流速防溢出阻尼后，机构对 **【{['主胜', '平局', '客胜'][max_idx]}】** 的实际赔付压力最敏感！")
...
# ==============================================================================
    ...
    st.success(f"📥 继承环境 | xG: **{mx_tg_val:.2f}** | K: **{mx_k_val:.2f}** | 让球赔率: **[{m1_let_w:.2f}, {m1_let_d:.2f}, {m1_let_l:.2f}]**")
...
# ==============================================================================
    ...
    if st.button("🚀 启动 365 剧本剥离", type="primary"):
        try:
            ...
            if override_index > 4.0: st.error(f"🦇 **【人工紧急避险熔断】** 内部变异系数爆表({override_index:.4f})！365精算师已断开AI自动平衡，针对特定冷门选项进行人工粗暴压水，极大内幕击杀可能！")
            else: st.success("💻 **【机器控盘期】** 四大盘口数学传动正常平稳，无剧烈人工干预痕迹。")
                
            if margin_diff > 0.0200: st.warning(f"🚧 **【极限缩表护盘】** 365临场暴力提升半全场抽水率(+{margin_diff*100:.4f}%)，庄家对该维度失去控盘自信！")
            ...
            def evaluate_m6_item(category, opt_name, delta, p_c, p_d):
                ...
                if category == 'std':
                    if opt_name == "主胜":
                        if delta > 0.015:
                            if delta_ah_up <= -0.015: return "🚨 【诱导陷阱】标胜造热，亚盘暗撤，防赢球输盘或爆冷！"
                            if delta_ah_up > 0.015 and delta_eh_d > 0.02: return "🚨 【赢一球壁垒】主胜/亚盘齐热，欧让平防守激增！防1球小胜。"
                            return "💎 【黄金共振】标亚同步极限施压，真实核心防守项。"
                        if delta < -0.015:
                            if delta_ah_up > 0.015: return "🕳️ 【深水反诱】标盘遭弃但亚盘诡异升水，庄家在下盘挖坑！"
                            return "📉 【真实抛弃】资金随势出逃，机构不再防守。"
                    elif opt_name == "平局" and delta > 0.015 and delta_std_w < -0.01: return "🚧 【冷平设防】主胜退潮资金大量涌入平局，被迫拉高平局防线。"
                    elif opt_name == "客胜" and delta > 0.02 and delta_std_w < -0.02: return "⚡ 【反转剧本】主胜崩塌，客胜强势吸筹，警惕爆冷反杀！"
                elif category == 'ah' and opt_name == "上盘水位":
                    if delta > 0.015 and delta_std_w <= -0.015: return "☠️ 【深水诱捕】强拉亚盘制造信心，标胜实则暴跌，骗筹上盘！"
                    if delta < -0.015 and delta_std_w > 0.015: return "🌊 【顺流泄洪】主胜利好下，亚盘全线放水诱买，极难穿盘。"
                    if delta > 0.02: return "🌋 【阻力飙升】单方面拉高亚盘阻力，庄家畏惧上盘打穿。"
                elif category == 'eh':
                    if "胜" in opt_name and delta < -0.015 and delta_std_w > 0.015: return "🚧 【穿盘铁幕】主胜大热但深盘防守暴跌，赢球输盘绝对预警！"
                    elif "平" in opt_name and delta > 0.02: return "🎯 【精准制导】让平防守大幅收紧，机构严防正好赢一球！"
                elif category == 'htft':
                    if opt_name == "胜/胜":
                        if delta_std_w > 0.015 and delta < -0.010 and ht_dw > 0.015: return "⏱️ 【时间轴暴露】主胜热但胜胜遭抛弃，疯抢“平/胜”！底牌在下半场！"
                        if delta > 0.015 and delta_std_w > 0.015: return "⚡ 【闪电战】与标盘共振，看好主队半场建立不可逆优势。"
                    elif opt_name == "平/胜" and delta > 0.015 and delta_std_w > 0.01: return "🔎 【剧本偏移】主胜大势下疯抢平胜，严防下半场绝杀！"
                    elif opt_name == "平/平" and delta > 0.02: return "🧊 【极限降温】重防平平，全场极度沉闷或 0-0 完场。"
            ...
            df_std_out = [{"选项": opts_m6_std[i], "初盘纯率": f"{p_std_c[i]:.4f}", "临场纯率": f"{p_std_d[i]:.4f}", "纯率增量(Δ)": f"{d_std[i]:.4f}", "深度战术定性": evaluate_m6_item('std', opts_m6_std[i], d_std[i], p_std_c[i], p_std_d[i])} for i in range(3)]
            st.dataframe(pd.DataFrame(df_std_out), hide_index=True, use_container_width=True)

            st.markdown("### 📉 365 亚洲让球盘 切片")
            df_ah_out = [{"选项": opts_m6_ah[i+1], "初盘纯率": f"{p_ah_c[i]:.4f}", "临场纯率": f"{p_ah_d[i]:.4f}", "纯率增量(Δ)": f"{d_ah[i]:.4f}", "深度战术定性": evaluate_m6_item('ah', opts_m6_ah[i+1], d_ah[i], p_ah_c[i], p_ah_d[i])} for i in range(2)]
            st.dataframe(pd.DataFrame(df_ah_out), hide_index=True, use_container_width=True)

            st.markdown("### 🥅 365 欧洲让球盘 切片")
            if np.sum(p_eh_c) > 0:
                df_eh_out = [{"选项": opts_m6_eh[i+1], "初盘纯率": f"{p_eh_c[i]:.4f}", "临场纯率": f"{p_eh_d[i]:.4f}", "纯率增量(Δ)": f"{d_eh[i]:.4f}", "深度战术定性": evaluate_m6_item('eh', opts_m6_eh[i+1], d_eh[i], p_eh_c[i], p_eh_d[i])} for i in range(3)]
                st.dataframe(pd.DataFrame(df_eh_out), hide_index=True, use_container_width=True)
            else: st.warning("➖ 欧让盘未录入数据，已安全跳过。")

            st.markdown("### ⏱️ 365 半/全场剧本 切片")
            df_ht_out = [{"选项": opts_m6_ht[i], "初盘纯率": f"{p_ht_c[i]:.4f}", "临场纯率": f"{p_ht_d[i]:.4f}", "纯率增量(Δ)": f"{d_ht[i]:.4f}", "深度战术定性": evaluate_m6_item('htft', opts_m6_ht[i], d_ht[i], p_ht_c[i], p_ht_d[i])} for i in range(9)]
...
# ==============================================================================
    ...
    st.caption("【多维流体动力学】摒弃死板单盘逻辑！联合标盘、让球盘实时流速，测算6大结果选项与0-7+球【弹性形变阈值】，精准定位盲区！")
    ...
    if st.button("🚀 启动全息非线性耦合扫描", type="primary", use_container_width=True):
        try:
            ...
            st.info("💡 **【模块九 - 雷达白话释义】**\n"
                    "• **协整环境特征**：系统自动扫描标盘、让球、进球数之间的异动。比如 `标平涨`+`让负涨`+`2球稳健` 会被直接判定为 **1-1隔离盲区**。\n"
                    "• **大盘预期张力(压迫)**：正数代表大盘（标盘 and 让球盘）资金正往这个进球数里挤压；负数代表资金在撤离。\n"
