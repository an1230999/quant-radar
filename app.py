...
if "FX2_V_FINAL_22" not in st.session_state:
    st.session_state.clear()
    st.session_state["FX2_V_FINAL_22"] = True
...
def get_poisson_pmf(k, lam):
    """纯 Python 原生算法替代 scipy.stats.poisson.pmf，彻底解决云端环境报错"""
...
# ================= 11. 模块五：V15 全息精算引擎 (无第三方库依赖版) =================
    ...
    if st.session_state[calc_key_m5]:
        ...
        with col_out1:
            st.markdown("#### ⚽ 进球数评分切片")
            df_g = pd.DataFrame({"选项": opts_m5_g, "理论值": [f"{x*100:.1f}%" for x in math_g], "实盘值": [f"{x*100:.1f}%" for x in pTC_g], "危险指数": scores_g, "全息定性": tags_g}).sort_values(by="危险指数", ascending=False)
            st.dataframe(df_g, hide_index=True)

        with col_out2:
            st.markdown("#### 🔵 半全场评分切片")
            df_h = pd.DataFrame({"选项": opts_m5_h, "理论值": [f"{x*100:.1f}%" for x in math_h], "实盘值": [f"{x*100:.1f}%" for x in pTC_h], "危险指数": scores_h, "全息定性": tags_h}).sort_values(by="危险指数", ascending=False)
