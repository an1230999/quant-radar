...
# ================= 1. 全局配置与UI优化 =================
st.set_page_config(page_title="FX2 量化对冲终端 [大结局至尊版]", layout="wide", page_icon="🏦")
...
# 💎 [Patch 01: 四分盘隐式投影解耦] 解决 -0.75, +0.25 盘口的底座数学坍塌
def calc_quarter_handicap_poisson(xg_h, xg_a, rho, k_val):
    _, _, _, _, _, P_mat = dixon_coles_full_matrix(xg_h, xg_a, rho)
    
    def get_slice_prob(k_int):
        pw = sum(P_mat[i, j] for i in range(8) for j in range(8) if i - j > -k_int)
        pd = sum(P_mat[i, j] for i in range(8) for j in range(8) if i - j == -k_int)
        pl = sum(P_mat[i, j] for i in range(8) for j in range(8) if i - j < -k_int)
        return np.array([pw, pd, pl])

    rem = abs(k_val) % 0.5
    if abs(rem - 0.25) < 0.001:
        if k_val < 0:
            k1, k2 = math.ceil(k_val * 2) / 2.0, math.floor(k_val * 2) / 2.0
        else:
            k1, k2 = math.floor(k_val * 2) / 2.0, math.ceil(k_val * 2) / 2.0
        p1 = get_slice_prob(int(k1)) if k1.is_integer() else get_slice_prob(int(round(k1)))
        p2 = get_slice_prob(int(k2)) if k2.is_integer() else get_slice_prob(int(round(k2)))
        return np.round(0.5 * p1 + 0.5 * p2, 4)
    else:
        return np.round(get_slice_prob(int(round(k_val))), 4)
...
# ==============================================================================
    ...
    if st.button("🚀 启动 365 剧本剥离", type="primary"):
        try:
            ...
            if override_index > 4.0: st.error(f"🦇 **【人工紧急避险熔断】** 内部变异系数爆表({override_index:.4f})！精算师已断开AI自动平衡，针对特定冷门选项进行人工粗暴压水，极大内幕击杀可能！")
