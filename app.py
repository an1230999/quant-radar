            ...
            for i in range(6):
                ...
                if is_lie:
                    ...
                    if flow >= 0.0250:
                        ...
                        scripts.append("存在微弱的受力偏移，建议结合大盘轨迹辅助研判。")
                    else:
...
# ==============================================================================
    ...
    st.caption("【全矩阵固化版】系统已锁死体彩官方全量31个比分项，自动静默忽略未填赔率，彻底阻断输入崩溃与数据丢失。")
    ...
    **如何看结果？**
    * 📊 **【看板一 (单比分查账)】**：如果某比分显示 `🚨 畸高诱捕墙`，这是庄家故意开高赔引诱你的毒药，**千万别买！** 如果是 `🛡️ 核心防波堤`，庄家极度害怕，**防冷定胆！**
    * 🧮 **【看板二 (总球数防假账)】**：对比球数和比分积分。**注意：如果你在比分台里漏填了该进球数下的主要比分，看板二会误报 `🚨 总池空壳抽水`。** 输全主要比分后，此处的报警即为庄家抽水铁证！
    """)
    st.markdown("---")
    ...
    if st.session_state[calc_key_m2]:
        try:
            ...
            # ====================================================================
            # 极度强壮的安全解析器：静默过滤无效赔率，重定向“胜其他”等汉字条目
            # ====================================================================
            for index, row in edited_scores_df.iterrows():
                ...
                # 如果赔率未填入或不合理(<=1.0)，则彻底静默忽略该行
                if odds_val <= 1.0: continue

                # “其他”类汉字项的物理拓扑重定向
                if score_str == "胜其他":
                    X, Y = 4, 3   # 虚拟坐标供理论纯率泊松提取用
                    G = 2         # 让球拓扑锚定虚拟净胜球
                    biao_key = "标胜"
                    goal_num = 7
                elif score_str == "平其他":
                ...
                if score_res > 0.025 and score_rmv > 0.05:
                    sc_verdict = "🚨 畸高诱捕墙"
                    sc_desc = "发生概率低，但体彩开出了不合理的超额赔付，纯属填仓诱网毒饵。"
                elif score_res < -0.025 and score_rmv < -0.05:
                    sc_verdict = "🛡️ 核心防波堤"
                    sc_desc = "发生概率高，精算师割肉降水阻击，属于机构死守的交割阵眼！"
                else:
                    sc_verdict = "⚪ 常规中枢"
                    sc_desc = "赔率处于合理博弈区间。"
            ...
            st.markdown("### 📊 看板一：微观行权比分自定义测谎表 (单点防爆雷)")
            ...
            st.markdown("---")
            st.markdown("### 🧮 看板二：【总球数期权池】与【细分行权比分】对账单 (全局查假账)")
            ...
            for idx, opt_g in enumerate(opts_m2_goals):
                ...
                if g_residual > 0.0300:
                    g_verdict = "🚨 总池空壳抽水"
                    g_desc = "总进球数概率被单边做高，但底层比分拒收流量！庄家单向克扣串关钱。"
                elif g_residual < -0.0300:
                    g_verdict = "🎁 细分漏网爆破"
                    g_desc = "比分积分远远挤爆了球数池！散户未注意的高赔比分里，机构在大面积筑墙泄洪。"
                else:
                    g_verdict = "⚖️ 微积分守恒"
                    g_desc = "总玩法池与细分期权交割单闭环平稳。"
            ...
            with c1_mx:
                ...
                if re_w < 0.65 and p_std_d_m2[0] > 0.40:
                    st.error("🚨 **【皮热骨冷·空壳死局】** 标盘主胜看起来极大，但下方录入的所有主胜核心比分根本没有流量支撑！排除主胜！")
                else:
                    st.success("✅ 大盘指数量价契合度符合离散分布。")

            with c2_mx:
                st.markdown("##### 🥅 让球映射大一统字典复核")
                st.write(f"• 临场让球纯率：[让胜 {p_let_d_m2[0]:.4f} | 让平 {p_let_d_m2[1]:.4f} | 让负 {p_let_d_m2[2]:.4f}]")
                st.write(f"• 比分路由积分：[让胜 {macro_rang_sums['让胜']:.4f} | 让平 {macro_rang_sums['让平']:.4f} | 让负 {macro_rang_sums['让负']:.4f}]")
                ...
                if K_int == -1 and goal_bucket_sums[3] > 0:
                    ...
                    if p_0_3_ratio > 0:
                        ...
                        st.warning(f"🎯 **【3球让负解耦成功】** 主让-1下，1-2与0-3均属让负。当前微观实际比率 1-2 vs 0-3 为 **{skew_ratio:.2f}倍**！若远低于理论8.0倍，说明精算师在用0-3冷门吸筹，诱骗散户去踩1-2死坑！")
