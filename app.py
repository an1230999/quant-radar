...
if "FX2_V_FINAL_23" not in st.session_state:
    st.session_state.clear()
    st.session_state["FX2_V_FINAL_23"] = True
...
st.title("🏦 FX2 全维量化对冲终端 (V14.0 满血版 + 原生V15全息引擎)")
...
# ================= 11. 模块五：V15 全息精算引擎 (扁平化安全排版 + 全量防错) =================
elif active_module == "🔭 模块五：V15 全息精算引擎":
    st.header(f"🔭 {current_match} - V15 全息量化精算实验室")
    st.caption("【终极防崩溃结构】完全修复 UI 嵌套闪退，内加百分百 NaN 值隔离屏障")
    ...
    st.markdown("### 📥 步骤一：录入多维赔率矩阵")
    # 🔥 核心修复一：用 Tabs 替换 columns，彻底杜绝 Streamlit 嵌套列崩溃BUG
    tab_g, tab_h = st.tabs(["⚽ 进球数矩阵录入", "🔵 半全场矩阵录入"])
    with tab_g: 
        res_m5_g = render_odds_grid("m5g", current_match, "进球数", opts_m5_g, cols_m5_new, init_m5_g)
    with tab_h: 
        res_m5_h = render_odds_grid("m5h", current_match, "半全场", opts_m5_h, cols_m5_new, init_m5_h)
    ...
    if st.session_state[calc_key_m5]:
        ...
        # 🔥 核心修复二：空值阻断（只要有任何一个数据未填，依然能运行下去）
