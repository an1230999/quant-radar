...
# ===================== 🔮 模块九：全息概率张力雷达 (跨盘口协整) =====================
# ==============================================================================
elif active_module == "🔮 模块九：全息概率张力雷达 (跨盘口协整)":
    st.header(f"🔮 {current_match} - M9 跨盘口全息非线性耦合期望对撞机")
    st.caption("【多维流体动力学】摒弃固定死限值！根据标盘与让球盘的实时大资金流向，全息求导计算0-7+进球数的【弹性形变阈值】。直接捕捉“让胜跌、2球涨、出冷平1-1”级别的高阶隔离盲区！")

    c9_1, c9_2, c9_3 = st.columns(3)
    with c9_1: m9_tg = safe_number_input("期望进球xG", f"m9_tg_{current_match}", 2.75, step=0.25)
    with c9_2: m9_hcp = safe_number_input("欧亚纯实力亚指", f"m9_hcp_{current_match}", -0.50, step=0.25)
    with c9_3: m9_k = safe_number_input("体彩实际让球K值 (正负皆可)", f"m9_k_{current_match}", -1.0, format="%.0f", step=1.0)

    st.markdown("### 📥 第 1 步：大盘（资金海）流速抓取")
    c9_std, c9_let = st.columns(2)
    with c9_std: res_m9_std = render_odds_grid("m9_std", current_match, "标盘 1X2", ["标胜", "标平", "标负"], ["初盘", "临场"], [[2.45, 2.32], [3.20, 3.20], [2.45, 2.60]])
    with c9_let: res_m9_let = render_odds_grid("m9_let", current_match, f"让球({int(m9_k)}) 1X2", ["让胜", "让平", "让负"], ["初盘", "临场"], [[5.50, 5.30], [4.10, 4.00], [1.42, 1.45]])

    st.markdown("---")
    st.markdown("### 📥 第 2 步：进球数（离散桶）标的输入")
    res_m9_g = render_odds_grid("m9_g", current_match, "体彩 0-7+ 球数", opts_m5_g, ["初盘", "临场"], init_m5_g)

    if st.button("🚀 启动全息非线性耦合张力测算", type="primary", use_container_width=True):
        try:
            ...
            shift_std = p_std_d - p_std_c  # 标盘流速
            shift_let = p_let_d - p_let_c  # 让球流速
            actual_shift_g = p_g_d - p_g_c # 进球数真实流速
            ...
            for g in range(8):
                ...
                for h in range(8):
                    for a in range(8):
                        if (h + a == g) or (g == 7 and h + a >= 7):
                            ...
                            # 资金体量势能权重分配: 标盘池海量(0.6), 让球池较深(0.4)
                ...
                if weight_sum > 0:
                    expected_tension[g] = tension_sum / weight_sum

            # 构建弹性形变区间 (基准容差 ±0.0150)
            z_base = 0.0150
            ...
            for g in range(8):
                ...
                # 全息协整战术定性
                v = ""
                if s > u: v = "🚨 突破弹性上限 (无视大盘阻力强拉，人工造热死坑！)"
                elif s < l: v = "🛡️ 击穿弹性下限 (顶着大盘逆流死防，绝对核心防波堤！)"
                else:
                    # 处于容差安全区间内，判断灰度动能
                    if t > 0.005 and s < -0.005:
                        v = "💎 逆向隔离盲区 (大盘推热该项，庄家却让其慢涨退水，冷门金矿！)"
                    elif t < -0.005 and s > 0.005:
                        v = "🕳️ 顺水推舟诱捕 (大盘资金在抛弃它，庄家故意降水引诱接盘！)"
                    else:
                        v = "⚪ 协整平衡 (随波逐流，无异常操作)"

                out_m9.append({
                    "进球数": opts_m5_g[g],
                    "静态纯率": f"{math_prob_g[g]:.4f}",
                    "预期张力偏移 (大盘引导)": f"{t:+.4f}",
                    "实际纯率偏移 (临场真实)": f"{s:+.4f}",
                    "动态弹性区间": f"[{l:+.4f} ~ {u:+.4f}]",
                    "全息定性裁决": v
            ...
            st.markdown("### 🧿 全息协整弹性矩阵对账单")
            st.dataframe(pd.DataFrame(out_m9), hide_index=True, use_container_width=True)
            
            st.info("💡 **【模块九 - 实战解密便签】**\n"
                    "• **预期张力偏移**：是系统根据 1X2 和 让球盘 的资金流向，通过泊松矩阵精准求导出的“这桶球数本该承受的资金压力”。正数代表大盘资金在往这个球数里涌，负数代表资金在撤离。\n"
                    "• **动态弹性区间**：不再是死板的数字！如果大盘资金在疯狂涌入（预期张力大），这桶球的上限和下限就会被**动态拉宽**。庄家给它稍微升降一点赔率，都不会被误判为诱导。\n"
                    "• **核心猎杀点【💎 逆向隔离盲区】**：大盘资金明明算出来该流向它，但它实际的纯率反而降低了（赔率缓慢升高了）。说明庄家在用极其隐蔽的手法，把资金往别处赶，**这就是你所说的 1-1（2球慢涨）打出的神级剧本！**")

        except Exception as e:
            st.error("🚨 模块九张力矩阵坍塌，请检查输入赔率！")
            st.code(traceback.format_exc())
