...
# ==============================================================================
    ...
    st.caption("【多维流体动力学】摒弃死板单盘逻辑！根据标盘(1X2)与让球盘的真实资金势能，动态合成0-7+球的弹性阈值。直接破解庄家『降水设坑、慢涨真出』的高阶隔离盲区。")
    ...
    with c9_3: m9_k = safe_number_input("体彩实际让球K值", f"m9_k_{current_match}", -1.0, format="%.0f", step=1.0)
    ...
    if st.button("🚀 启动全息非线性耦合张力雷达", type="primary", use_container_width=True):
        try:
            ...
            shift_g = p_g_d - p_g_c        # 进球数流速

            # 2. 联合张力审查矩阵（大盘6大核心项扫描）
            st.markdown("#### 🚨 核心关联项联合审查便签")
            
            # 标盘审查
            for idx, name in enumerate(["主胜", "平局", "客负"]):
                flow = shift_std[idx]
                if pd.isna(flow): continue
                if flow > 0.02:
                    if name == "主胜" and shift_let[0] > 0.01:
                        st.success(f"✅ **【量价齐升直通车】** 散户与主力在『标胜』方向发生超级共振。若此时大比分进球数（3/4球）水降，可无脑追主队穿盘大胜！")
                    elif name == "主胜" and shift_g[1] < -0.015 and shift_g[2] < -0.015:
                        st.error(f"🚨 **【虚假繁荣路径隔离】** 标盘主胜看似暴热，但关联核心比分的池子（1球/2球）正在被庄家疯狂抽干！极大概率是防冷平或主负的诱导陷阱！")
                    elif name == "平局" and shift_g[0] < -0.02 and shift_g[2] < -0.02:
                        st.warning(f"🚧 **【双重死防底线】** 标平暴跌，同时 0球 和 2球（1-1专属关联）也在暴跌。庄家极其恐惧平局，明牌封锁利润！")
                elif flow < -0.015:
                    if name == "平局" and shift_let[2] < -0.01:
                        if shift_g[2] > 0.005 and shift_g[2] < 0.025:
                            st.info(f"💎 **【神级路径隔离盲区】** 庄家微抬平局赔率诱导买胜，实际把 1-1（关联2球与让负）藏在了无人问津的高赔孤岛。强力看好 1-1 盲区打出！")

            # 让球审查
            for idx, name in enumerate(["让胜", "让平", "让负"]):
                flow = shift_let[idx]
                if pd.isna(flow): continue
                if flow > 0.02:
                    if name == "让胜" and shift_std[0] < -0.01 and shift_g[3] > 0.015:
                        st.error(f"🕳️ **【暗水偷袭老鼠仓】** 标盘主胜不热，但博取穿盘的『让胜』突然狂热，小心游资已知内幕，随时有剧本大胜！")
                elif flow > 0.005 and flow < 0.02:
                    if name == "让胜" and shift_g[2] < -0.005:
                        st.success(f"🛡️ **【极限微操挤压区】** 让胜在微降吸筹，迫使 2球 资金干涸慢涨退水。不要被涨水吓到，这正是 2球 打出的黄金安全期！")

            # 3. 动态引力引擎：求解 0-7+ 球数弹性边界
            ...
            for g in range(8):
                ...
                if s > u: v = "🚨 涨破动态上限 (无视大盘阻力逆势强拉，坚决避坑)"
                    ...
                    if t > 0.005 and s < -0.005: v = "💎 逆向隔离金矿 (大盘推热，庄家却让其退水，防守盲区！)"
                    elif t < -0.005 and s > 0.005: v = "🕳️ 顺水推舟毒药 (大盘抛弃，庄家却故意降赔，骗接盘！)"
                    else: v = "⚪ 势能守恒 (符合大盘流动)"
            ...
            st.info("💡 **【进球数雷达白话释义】**\n"
                    "• **大盘预期张力**：正数代表大盘（标盘和让球盘）的资金正在向这个进球数方向挤压；负数代表资金在撤离。\n"
                    "• **动态弹性容差区**：系统根据大盘挤压度，实时拓宽或收窄的判定红线。\n"
                    "• **核心猎杀点【💎 逆向隔离金矿】**：这正是你所说的“慢涨反而打出”。大盘明明逼着它降赔，它偏偏在涨。说明庄家在用极其隐蔽的手法把资金往别处赶，这里藏着顶级冷门！")
