...
st.title("🏦 FX2 全维量化对冲终端 (V14.0满血版 + V15全息隔离舱)")
...
# ================= 11. 模块五：V15 全息精算引擎 (彻底去Pandas强转版) =================
    ...
    c1, c2 = st.columns(2)
    with c1: res_m5_g = render_odds_grid("m5g", current_match, "进球数矩阵", opts_m5_g, cols_m5_new, init_m5_g)
    with c2: res_m5_h = render_odds_grid("m5h", current_match, "半全场矩阵", opts_m5_h, cols_m5_new, init_m5_h)
    ...
    if st.session_state[calc_key_m5]:
        ...
        # 🔥 全局 Try-Except 异常捕获机制：一旦报错，界面不崩溃，而是显示具体原因
        try:
            ...
            m3 = 0.0
            if not np.isnan(pTC_g).any() and not np.isnan(cons_g).any():
                m3 = float((pTC_g[1]+pTC_g[3]+pTC_g[5]+pTC_g[7]) - (cons_g[1]+cons_g[3]+cons_g[5]+cons_g[7]))
                m3 = round(m3, 4)
            ...
            # 终极保护层：如果有任何运算或系统报错，拦截并打印，绝对不让网页崩溃！
            st.error("🚨 **V15全息引擎保护机制触发：捕捉到环境或运算异常**")
            st.warning(f"直接报错原因：`{str(e)}`")
            st.info("💡 提示：这通常是因为输入了非法的负数、极端的0值，或者是由于 Streamlit 云端渲染机制导致的闪退。不用担心，前四个模块的功能完全不受影响！")
            with st.expander("🛠️ 展开查看详细代码追踪报错 (Traceback)"):
