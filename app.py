...
st.markdown("""<style>.block-container{padding-top:1rem;padding-bottom:0rem;} h1{font-size:1.8rem;margin-bottom:0rem;} h2{font-size:1.4rem;} h3{font-size:1.1rem;color:#4CAF50;} div[role="radiogroup"]{padding-bottom:10px;border-bottom:1px solid #444;} div[role="radiogroup"]>label{padding-right:20px;font-weight:bold;} body{line-height:1.25;}</style>""", unsafe_allow_html=True)
...
def calc_pure_prob_array(arr):
    ...
    return np.round((1.0 / arr) / np.nansum(1.0 / arr), 4)

def calc_liab_shift(prob_c, odds_c, prob_d, odds_d):
    return np.round(prob_d * odds_d - prob_c * odds_c, 4)
...
def dixon_coles_full_matrix(lambda_, mu_, rho_, is_knockout=False):
    def poisson_pmf_array(lam, max_k):
        ...
        return pmf
    px, py = poisson_pmf_array(lambda_, 15), poisson_pmf_array(mu_, 15)
    P = np.outer(px, py)
    P[0, 0] *= (1 - lambda_ * mu_ * rho_); P[1, 0] *= (1 + lambda_ * rho_)
    P[0, 1] *= (1 + mu_ * rho_); P[1, 1] *= (1 - rho_)
    P = np.clip(P, 0, 1)
    if is_knockout: P[0, 0] *= 1.35; P[1, 1] *= 1.25; P[2, 2] *= 1.10
    if P.sum() > 0: P = P / P.sum() 
    P_col = np.zeros((8, 8))
    P_col[:7, :7] = P[:7, :7]
    P_col[7, :7], P_col[:7, 7] = np.sum(P[7:, :7], axis=0), np.sum(P[:7, 7:], axis=1) 
...
def safe_extract_array(data_list):
    out = []
    for x in data_list:
        try: v = float(x); out.append(v if not math.isnan(v) else 0.0)
...
def safe_number_input(label, state_key, default_val, format="%.4f", step=0.0010, **kwargs):
    ...
    except: clean_val = float(default_val)
...
def render_odds_grid(module_key, match_id, wl, options, col_names, init_data):
    st.markdown(f"### 📥 {wl}")
    grid_cols = st.columns([1.5] + [1] * len(col_names))
    ...
    for i, opt in enumerate(options):
        cols = st.columns([1.5] + [1] * len(col_names))
        ...
        for j, cname in enumerate(col_names):
            ...
            except: clean_val = float(init_data[i][j])
            st.session_state[state_key] = clean_val
...
# ================= 5. 底座初始参数 =================
opts_m1 = ["标盘-胜", "标盘-平", "标盘-负", "让盘-胜", "让盘-平", "让盘-负"]; cols_m1 = ["初盘", "临场"]
...
for k in list(st.session_state.keys()):
    if k.startswith("m2_shadow_"):
        ...
        if not (is_viewing_m2 and (current_match in k) and (current_m2_wl_viewing in k)) and base_k in st.session_state:
            st.session_state[base_k] = st.session_state[k].copy()
    elif k.startswith("m8_shadow_"):
        base_k8 = k.replace("m8_shadow_", "m8_base_sc_")
        if not (is_viewing_m8 and (current_match in k)) and base_k8 in st.session_state:
            st.session_state[base_k8] = st.session_state[k].copy()

# ==============================================================================
# ===================== 🎯 模块七：全息连通器·深盘猎杀终端 =====================
# ==============================================================================
if active_module == "🎯 模块七：全息连通器·深盘猎杀终端 (V30)":
    st.header(f"🎯 {current_match} - V30 深盘猎杀显微镜")
    st.caption("【微创大修版】修复深盘K值硬编码。动态微积分泊松桥接，精准剥离任何深盘的幽灵标盘1X2。")

    col_e1, col_e2, col_e3 = st.columns(3)
    with col_e1: m7_tg = safe_number_input("全场期望xG", f"m7_tg_{current_match}", 3.00, step=0.25)
    with col_e2: m7_hcp = safe_number_input("初始实力亚指", f"m7_hcp_{current_match}", -1.50, step=0.25)
    with col_e3: m7_k = safe_number_input("实际让球数K (可填任意值)", f"m7_k_{current_match}", -2.0, format="%.0f", step=1.0)

    is_all_std_closed = st.toggle("🚫 【官方未售标盘】(通过连通器代偿逆向还原)", value=True)
    ...
    with col_std:
        if is_all_std_closed: st.warning("🔒 官方屏蔽标盘，后台动态微积分逆向解耦。")
        else: res_std = render_odds_grid("m7std", current_match, "体彩【标盘】", opts_std, ["初盘", "临场"], [[1.15, 1.10], [6.50, 7.00], [15.0, 19.0]])
    ...
    if st.session_state[calc_key_m7]:
        try:
            ...
            # 【动态微积分桥接算法】取代死板的 K=2
            bridge_val = 0.0
            if K_int < -1: bridge_val = sum(P_mat[h, a] for h in range(8) for a in range(8) if 0 < h - a < abs(K_int))
            elif K_int > 1: bridge_val = sum(P_mat[h, a] for h in range(8) for a in range(8) if 0 < a - h < K_int)
            ...
            if is_all_std_closed:
                if K_int < 0: 
                    pw_c, pw_d = p_let_c[0]+p_let_c[1]+bridge_val, p_let_d[0]+p_let_d[1]+bridge_val
                    ...
                    pl_c, pl_d = p_let_c[2]+p_let_c[1]+bridge_val, p_let_d[2]+p_let_d[1]+bridge_val
            ...
            if K_int < 0:
                residuals[0], residuals[3], residuals[4] = round(p_all_d[0]-(p_all_d[3]+p_all_d[4]+bridge_val),4), round(p_all_d[3]-(p_all_d[0]-p_all_d[4]-bridge_val),4), round(p_all_d[4]-(p_all_d[0]-p_all_d[3]-bridge_val),4)
                residuals[5], residuals[1], residuals[2] = round(p_all_d[5]-(p_all_d[1]+p_all_d[2]+bridge_val),4), round(p_all_d[1]-(p_all_d[5]-p_all_d[2]-bridge_val),4), round(p_all_d[2]-(p_all_d[5]-p_all_d[1]-bridge_val),4)
            elif K_int > 0:
                residuals[3], residuals[0], residuals[1] = round(p_all_d[3]-(p_all_d[0]+p_all_d[1]+bridge_val),4), round(p_all_d[0]-(p_all_d[3]-p_all_d[1]-bridge_val),4), round(p_all_d[1]-(p_all_d[3]-p_all_d[0]-bridge_val),4)
                residuals[2], residuals[4], residuals[5] = round(p_all_d[2]-(p_all_d[4]+p_all_d[5]+bridge_val),4), round(p_all_d[4]-(p_all_d[2]-p_all_d[5]-bridge_val),4), round(p_all_d[5]-(p_all_d[2]-p_all_d[4]-bridge_val),4)
            ...
            for i in range(6):
                ...
                lie_r_show.append(f"{res:+.4f} (🔴造热)" if res > dyn_thresh else (f"{res:+.4f} (🟢筑墙)" if res < -dyn_thresh else f"{res:+.4f} (⚪容差)"))
                rmv_show.append(f"{r*100:+.2f}% (🔴诱导)" if r > 0.04 else (f"{r*100:+.2f}% (🟢核心)" if r < -0.04 else f"{r*100:+.2f}% (⚪常规)"))
                
                if res > dyn_thresh and r > 0.04: verdicts.append("🚨 镜像畸高 (造热死坑)"); scripts.append("跨盘概率虚假拔高，精算师制造稳赢假象。")
                elif res < -dyn_thresh and r < -0.04:
                    if flow > -0.01: verdicts.append("💎 全息闭环暗水王"); scripts.append("承接对冲纯率，全场第一单挑位！")
            ...
            st.markdown("### 📊 V30 动态微积分重构·大终局体检表")
            ...
            r1.metric("⚖️ 胜负势能张力", f"{(p_all_d[0]-p_all_d[2])*100:+.1f}%")
            r2.metric("🕳️ 卡盘绝杀绞杀比", f"{bridge_val/max(p_let_d[1],0.0001):.2f} 倍", help="卡盘积分 / 让平流速")
        except Exception as e: st.error("🚨 模块七异常。"); st.code(traceback.format_exc())
            # ==============================================================================
# ===================== 🔥 模块X：全息综合引擎 =====================
# ==============================================================================
elif active_module == "🔥 模块X：全息综合引擎 (M1+M3+M4)":
    st.header(f"🔥 {current_match} - 模块X：综合引擎")
    ...
    def render_module_x_ui(wl, match_id):
        ...
        if st.button(f"🚀 执行 {wl} 综合精算", type="primary", key=f"btn_{calc_key}", use_container_width=True): st.session_state[calc_key] = True
        ...
        if st.session_state[calc_key]:
            ...
            st.dataframe(pd.DataFrame({"选项": opts_m1, "初纯率": p_c, "临纯率": p_d, "Delta动能": delta}).fillna(""), hide_index=True, use_container_width=True)
            ...
            xg_d = round((mx_tg-mx_hm)/2 - (mx_tg+mx_hm)/2, 4)
            st.info(f"🌪️ 时空撕裂度: **{round(xg_d - (-mx_hb), 4)}** (泊松净胜 {xg_d} vs 盘口 {-mx_hb})")
...
# ==============================================================================
    ...
    st.caption("注：本模块对应【体彩竞彩 31项固定比分】，北单用户请移步至M8。")
    ...
    if st.button("🚀 启动体彩多维拓扑对账", type="primary"):
        try:
            ...
            for _, row in ed_df.iterrows():
                ...
                else: X, Y = int(sc_s.split("-")[0]), int(sc_s.split("-")[1]); gn = min(X+Y, 7)
...
# ==============================================================================
    ...
    with tab_m8_a:
        ...
        if st.button("🚀 执行 Tab1 基础大盘对撞", key=f"btn_m8_a_{current_match}", type="primary"):
            ...
            st.info("💡 **【大盘分析白话说明】**\n"
                    ...
                    "如果为 **负数**，说明散户资金**严重踩踏**，奖金被严重摊薄稀释，属于【赔本赚吆喝】，应坚决规避。")
    ...
    with tab_m8_b:
        ...
        st.caption("【积分修正完成】：胜/平/负其他 已完美涵盖 5-0、7+球等所有边缘比分，并且引入了晚高峰稀释算子算出真实EV。")
        ...
        if st.button("🚀 执行 25项比分显微审计", key=f"btn_m8_b_{current_match}", type="primary"):
            ...
            sp_pred_arr = sps_arr * (1.0 - norm_fav_arr * m8_dil)
            ...
            for i, r in e_b8.iterrows():
                ...
                ev_sc = round(p_m * sp_pred_arr[i] - 1.0, 4)
            ...
            if o8: 
                ...
                st.info("💡 **【比分分析白话说明】**\n"
                        ...
                        "• **买入划算度(EV) 为 正数 (💎)**：代表这个比分极有可能打出，但国内散户**根本没买**。它的SP值处于极度虚高的暴利状态，强烈建议买入！\n"
                        "• **买入划算度(EV) 为 负数 (🚨)**：代表国内散户瞎买，把这个比分的池子挤爆了，被稀释后性价比极差，坚决不要买。")
            else: st.warning("请在上方填入至少1个比分的SP值。")

    # ==================== M8 / Tab 3：克隆双胞胎套利 ====================
    with tab_m8_c:
        st.markdown("#### ⚖️ 「总进球数」 vs 「上下单双」无脑白嫖套利")
        st.caption("【数学越界修复】：已在后台利用泊松剥离出 7+球 里的奇数与偶数比例，杜绝假套利！")
        ...
        if st.button("🚀 启动双胞胎克隆套利扫描", key=f"btn_m8_c_{current_match}", type="primary"):
            ...
            r_odd = p7_odd / max(p7_odd + p7_even, 0.0001)
            r_even = 1.0 - r_odd
            pg_up_even = round(p_g[4] + p_g[6] + p_g[7] * r_even, 4)
            pg_up_odd = round(p_g[3] + p_g[5] + p_g[7] * r_odd, 4)
