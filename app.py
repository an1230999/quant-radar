...
if active_module == "🏆 模块七：世界杯全息狙击终端 (经典TC主导)":
    ...
    st.caption("【经典回滚】输入核心比分进行极限防线反套。")
    ...
    if st.session_state[calc_key_m7]:
        ...
        try:
            ...
            m4_alerts = ["⚪ 常规"] * 3
            for i in range(3):
                if shift_tc[i] > 0.03 and shift_365[i] < 0.01: m4_alerts[i] = "💣 体彩单边资金堰塞湖 (杀猪诱导)"
                elif shift_tc[i] > 0.03 and shift_365[i] > 0.02: m4_alerts[i] = "🛡️ 全球同步重注涌入 (真实设防)"
                elif shift_tc[i] < -0.03: m4_alerts[i] = "🧊 体彩极速退守放水"
            ...
            if d_tc_365 > 0.05 and d_hk_365 <= 0.03: st.error("🌪️ **【剧本一：大陆情绪陷阱】** 365与马会保持冷静，体彩利用球迷情绪单方面疯狂造热！凡体彩大幅降水项全部为诱捕毒饵！")
            elif d_hk_365 > 0.05 and d_tc_365 <= 0.03: st.warning("🦇 **【剧本二：亚洲核心防范】** 马会独家大幅收紧防线，脱离全球精算轨道。极大可能代表亚洲核心内幕意图，跟进马会方向！")
            elif d_tc_hk <= 0.025 and d_tc_365 > 0.045: st.info("🚧 **【剧本三：亚洲联合壁垒】** 体彩与马会高度重合，联合对抗365模型。亚洲区庄家真实防范意图明显，该区域项打出概率极高，顺势而为！")
            elif d_tc_365 <= 0.035 and d_hk_365 <= 0.035: st.success("✅ **【剧本四：全息共振】** 三方机构纯概率空间协方差极小，步调完全一致。市场无情绪扭曲，请直接依靠本中控台的数据流速自行定夺！")
            else: st.markdown("> *当前三极管处于混沌状态，各方资金剧烈换手中，请密切关注上方的敞口警报。*")
        except Exception as e:
            st.error("🚨 模块七运行异常，请检查填写数据。")
            st.code(traceback.format_exc())

# ==============================================================================
# ===================== ⚔️ 模块一：欧亚大盘体系 (精准收口+不连续耦合传动版) =====================
# ==============================================================================
elif active_module == "⚔️ 模块一：欧亚大盘体系 (精准收口版)":
    st.header(f"⚔️ {current_match} - 欧亚大盘体系 (精准收口版)")
    st.caption("【高阶重构说明】引入自适应不连续耦合传动模型(CTD)与概率质量相对变异度(RMV)，完美阻击让1/让2及受让全盘口伪装。")
    ...
    def render_main_handicap_ui(wl, match_id):
        ...
        st.markdown("#### ⚙️ 欧亚全息传动拓扑底座配置")
        c1, c2, c3 = st.columns(3)
        with c1: m1_tg = st.number_input("进球盘期望 (大小球基准)", value=2.50, step=0.25, format="%.2f", key=f"m1_tg_{match_id}_{wl}")
        with c2: m1_hcp = st.number_input("让球盘基准 (主队初始亚指)", value=-0.50, step=0.25, format="%.2f", key=f"m1_hcp_base_{match_id}_{wl}")
        with c3: m1_k = st.number_input("体彩官方实际让球数 (K, 主让为负, 受让为正)", value=-1.0, step=1.0, format="%.0f", key=f"m1_k_{match_id}_{wl}")
        ...
        if st.button(f"🚀 执行 {wl} 耦合精算", type="primary", key=f"btn_{calc_key}"): st.session_state[calc_key] = True
        ...
        if st.session_state[calc_key]:
            ...
            # --- 不连续刚性全息耦合传动解构 (CTD & RMV 核心算法) ---
            xg_h, xg_a = (m1_tg - m1_hcp) / 2, (m1_tg + m1_hcp) / 2
            _, _, _, _, _, P_matrix = dixon_coles_full_matrix(xg_h, xg_a, -0.15)
            
            p_theo_std_w = sum(P_matrix[h, a] for h in range(8) for a in range(8) if h > a)
            p_theo_std_d = sum(P_matrix[h, a] for h in range(8) for a in range(8) if h == a)
            p_theo_std_l = sum(P_matrix[h, a] for h in range(8) for a in range(8) if h < a)
            
            K_int = int(m1_k)
            p_theo_let_w = sum(P_matrix[h, a] for h in range(8) for a in range(8) if h - a > -K_int)
            p_theo_let_d = sum(P_matrix[h, a] for h in range(8) for a in range(8) if h - a == -K_int)
            p_theo_let_l = sum(P_matrix[h, a] for h in range(8) for a in range(8) if h - a < -K_int)
            
            p_theo_all = np.array([p_theo_std_w, p_theo_std_d, p_theo_std_l, p_theo_let_w, p_theo_let_d, p_theo_let_l])
            residuals = np.round(prob_d - p_theo_all, 4)
            
            rmv = np.zeros(6)
            for i in range(6):
                if prob_d[i] > 0: rmv[i] = round(residuals[i] / prob_d[i], 4)
                
            # 计算传动位移桥接项(不连续过渡矢量)
            bridge_val = 0.0
            if K_int < 0:
                bridge_val = sum(P_matrix[h, a] for h in range(8) for a in range(8) if 0 < h - a < -K_int)
                ctd_val = round((prob_d[3] + prob_d[4]) - (prob_d[0] - bridge_val), 4)
            else:
                bridge_val = sum(P_matrix[h, a] for h in range(8) for a in range(8) if 0 < a - h < K_int)
                ctd_val = round(prob_d[3] - (prob_d[0] + prob_d[1] + bridge_val), 4)
                
            t_open, w_traj, aa_hedge = [""] * 6, [""] * 6, [""] * 6
            ...
            for i in range(6):
                # 1. 连续渐变态审查
                if residuals[i] > 0.0300: t_open[i] = "🔺 镜像过度拉伸"
                elif residuals[i] < -0.0300: t_open[i] = "🔻 镜像非线性压缩"
                else: t_open[i] = "⚪ 物理轨道重合"
                
                # 2. 抽水非均匀荷载校正
                if dev[i] < -0.1000: w_traj[i] = "🛡️ 极限自保压水的"
                elif dev[i] > 0.1000: w_traj[i] = "🚨 放弃设防放水"
                else: w_traj[i] = "⚪ 常规换手"
                ...
                if dev[i] < -0.1200:
                    aa_hedge[i] = "🛡️ 真实自保防线"
                elif dev[i] > 0.1200 and delta[i] < -0.0100:
                    aa_hedge[i] = "🧊 极限绞杀(被弃核心)"
                elif residuals[i] > 0.0200 and dev[i] > 0.0200:
                    if is_dominant: aa_hedge[i] = "🚨 致命背离(造热核心)"
                    else: aa_hedge[i] = "🟡 防守溢出(非主线)"
                elif residuals[i] < -0.0200 and dev[i] < 0.0200:
                    if is_dominant: aa_hedge[i] = "🎁 暗度陈仓(核心轴)"
                    else: aa_hedge[i] = "⚪ 泄洪波及(非主线)"
                else:
                    if rmv[i] > 0.0800: aa_hedge[i] = "⚠️ 局部资产热泡沫"
                    elif rmv[i] < -0.0800: aa_hedge[i] = "🕸️ 静态跨盘诱捕网"
                    else: aa_hedge[i] = "⚪ 常规轻微震荡"

            out_main = pd.DataFrame({
                "选项": opts_m1, "初纯净率": prob_c, "临场纯率": prob_d, "动量(Δ)": delta, 
                "相对偏离(RMV)": rmv, "镜像残差": residuals, "传动状态审计": t_open, 
                "抽水荷载校正": w_traj, "全息嵌套裁决": aa_hedge
            })
            st.markdown("### 📊 欧亚大盘6维全息不连续嵌套透视矩阵")
            st.dataframe(out_main.style.format({"初纯净率": "{:.4f}", "临场纯率": "{:.4f}", "动量(Δ)": "{:.4f}", "相对偏离(RMV)": "{:.4f}", "镜像残差": "{:.4f}"}), hide_index=True, use_container_width=True)

            st.write("")
            st.markdown("### 🥇 欧亚刚性链传动矢量审计看板 (CTD)")
            col_c1, col_c2 = st.columns(2)
            col_c1.metric("⚡ 当前传动位移形变度 (CTD)", f"{ctd_val:.4f}")
            
            if abs(ctd_val) > 0.0350:
                col_c2.error("🚨 **【链条发生刚性撕裂点断裂】** 大陆标盘资金分布与底层让盘防线严重拉扯，精算师正在用极其畸形的空间割裂诱导大户。强烈建议使用排除法，反打跨盘被动泄洪项！")
            else:
                col_c2.success("✅ **【链条传动平稳】** 标盘传动至让盘的概率质量流速完全在物理合理对冲区间，市场传动无欺骗性宏观断层。")
...
# ==============================================================================
    ...
    if st.session_state[calc_key_m6]:
        ...
        try:
            ...
            if override_index > 4.0: st.error(f"🦇 **【人工紧急避险熔断】** 内部变异系数爆表({override_index:.4f})！365精算师已断开AI自动平衡进行人工粗暴压水，该项有极大内幕击杀可能！")
            ...
            def evaluate_m6_item(category, opt_name, delta, p_c, p_d):
                ...
                if category == 'std':
                    if opt_name == "主胜":
                        if delta > 0.015:
                            if delta_ah_up <= -0.015: return "🚨 【诱导陷阱】标盘疯狂造热主队，但亚盘暗中撤防，赢球输盘或爆冷！"
                            if delta_ah_up > 0.015 and delta_eh_d > 0.02: return "🚨 【刚好赢一球壁垒】主胜/亚盘齐热，但欧让平防守激增！防主队1球小胜输盘。"
            ...
            df_std_out = []
            for i in range(3):
                df_std_out.append({"选项": opts_m6_std[i], "初盘纯率": f"{p_std_c[i]:.4f}", "临场纯率": f"{p_std_d[i]:.4f}", "纯率增量(Δ)": f"{d_std[i]:.4f}", "深度战战判定": evaluate_m6_item('std', opts_m6_std[i], d_std[i], p_std_c[i], p_std_d[i])})
            st.dataframe(pd.DataFrame(df_std_out), hide_index=True, use_container_width=True)

            st.markdown("### 📉 365 亚洲让球盘 切片")
            df_ah_out = []
            for i in range(2):
                df_ah_out.append({"选项": opts_m6_ah[i+1], "初盘纯率": f"{p_ah_c[i]:.4f}", "临场纯率": f"{p_ah_d[i]:.4f}", "纯率增量(Δ)": f"{d_ah[i]:.4f}", "深度战战判定": evaluate_m6_item('ah', opts_m6_ah[i+1], d_ah[i], p_ah_c[i], p_ah_d[i])})
            ...
            if np.sum(p_eh_c) > 0:
                df_eh_out = []
                for i in range(3):
                    df_eh_out.append({"选项": opts_m6_eh[i+1], "初盘纯率": f"{p_eh_c[i]:.4f}", "临场纯率": f"{p_eh_d[i]:.4f}", "纯率增量(Δ)": f"{d_eh[i]:.4f}", "深度战战判定": evaluate_m6_item('eh', opts_m6_eh[i+1], d_eh[i], p_eh_c[i], p_eh_d[i])})
            ...
            df_ht_out = []
            for i in range(9):
                df_ht_out.append({"选项": opts_m6_htft[i], "初盘纯率": f"{p_ht_c[i]:.4f}", "临场纯率": f"{p_ht_d[i]:.4f}", "纯率增量(Δ)": f"{d_ht[i]:.4f}", "深度战战判定": evaluate_m6_item('htft', opts_m6_htft[i], d_ht[i], p_ht_c[i], p_ht_d[i])})
...
# ==============================================================================
    ...
    source_wl = st.radio("📡 选择数据源：", ["浅水区", "中水区", "深水区"], horizontal=True)
    tab_a, tab_b, tab_c = st.tabs(["🔍 亚盘 vs xG 撕裂检测", "🏦 机构暗水剥离", "⚖️ 荷兰式绝对零和对冲器"])
    with tab_a:
        ...
        try:
            ...
            if mismatch >= 0.4: st.success("✅ **主队深度价值：** 机构开盘极其保守！")
            elif mismatch <= -0.4: st.error("🚨 **极致诱杀陷阱：** 机构强开深盘造热，坚决去下盘/客队不败！")
        except KeyError: st.error("⚠️ 请先输入大盘数据。")
    with tab_b:
        st.markdown("### 🏦 机构真实赔付敞口与暗水探测")
        try:
            d_odds = [float(st.session_state.get(f"m1_{current_match}_{source_wl}_r0_c1", 2.32)), float(st.session_state.get(f"m1_{current_match}_{source_wl}_r1_c1", 3.20)), float(st.session_state.get(f"m1_{current_match}_{source_wl}_r2_c1", 2.60))]
            implied = 1.0 / np.array(d_odds)
            margin = np.sum(implied) - 1
            fair_prob = implied / (1 + margin)
            liability = fair_prob * np.array(d_odds)
        ...
        except: st.warning("⚠️ 请先在大盘确认临场赔率。")
    with tab_c:
