            ...
            for i in range(6):
                ...
                if is_lie:
                    ...
                    if flow >= 0.0250:
                        ...
                        scripts.append("存在微弱的受力偏移，建议结合模块一的大盘轨迹研判。")
                    else:
...
# ==============================================================================
    ...
    * 🧮 **【看板二 (总球数防假账)】**：对比球数和比分积分。**注意：如果你没有在比分台里输入对应进球数（比如0球）的比分（如0-0），看板二必然会因为没有数据而报警 `🚨 总池空壳抽水`。** 只有当对应进球数下的主要比分都输全了，这里的报警才是真正的庄家抽水铁证！
    ...
    # 直接读取并赋值给 session_state 保存持久化，完美解决切模块丢失数据的问题
    current_df = st.data_editor(
        st.session_state[base_df_key], 
        num_rows="dynamic", 
        use_container_width=True,
        key=f"m2_editor_widget_{current_match}_{m2_source_wl}"
    )
    st.session_state[base_df_key] = current_df
    edited_scores_df = st.session_state[base_df_key]
