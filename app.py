...
if "FX2_V_FINAL_24" not in st.session_state:
    st.session_state.clear()
    st.session_state["FX2_V_FINAL_24"] = True
...
st.title("🏦 FX2 全维量化对冲终端 (V15 终极防崩溃全息版)")
...
cols_m5_new = ["365赔率", "香港马会", "中国体彩"]
init_m5_g = [[15.0, 13.0, 14.0], [5.5, 5.3, 5.8], [4.0, 3.75, 3.7], [4.0, 3.7, 3.4], [5.5, 4.6, 4.8], [9.0, 6.8, 7.8], [17.0, 13.0, 15.0], [21.0, 18.0, 23.0]]

opts_m5_h = ["胜/胜", "胜/平", "胜/负", "平/胜", "平/平", "平/负", "负/胜", "负/平", "负/负"]
...
# ================= 11. 模块五：V15 全息精算引擎 (独立防弹闭环架构) =================
    ...
    def m5_render_grid(module_key, match_id, wl, options, col_names, init_data):
        ...
        for i, opt in enumerate(options):
            ...
            for j, cname in enumerate(col_names):
                ...
                if base_key not in st.session_state:
                    st.session_state[base_key] = init_data[i][j]
    ...
    def generate_poisson_baselines(tg, hcp):
        ...
        for i in range(8):
            for j in range(8):
                p_ht, p_sh = get_poisson_pmf(i, ht_xg_h) * get_poisson_pmf(j, ht_xg_a), get_poisson_pmf(i, sh_xg_h) * get_poisson_pmf(j, sh_xg_a)
    ...
    def calculate_score(tc_pure, eu_pure, math_pure, is_in_trend, is_hk_cliff, is_poison_bait):
        score = 0
        dev_eu = (tc_pure / eu_pure) - 1 if eu_pure and eu_pure > 0 and not pd.isna(tc_pure) else 0
        dev_math = (tc_pure / math_pure) - 1 if math_pure and math_pure > 0 and not pd.isna(tc_pure) else 0
    ...
    with st.expander("⚙️ 引擎底座参数 (点击展开设定大盘基准)", expanded=True):
        ...
        st.info("💡 数学引擎将根据上述盘口，自动生成比赛绝对公平概率，作为第三方照妖镜。")
        
    res_m5_g = m5_render_grid("m5g", current_match, "进球数矩阵", opts_m5_g, cols_m5_new, init_m5_g)
    st.write("---")
    res_m5_h = m5_render_grid("m5h", current_match, "半全场矩阵", opts_m5_h, cols_m5_new, init_m5_h)
    ...
    if st.session_state[calc_key_m5]:
        ...
        tcW = pTC_h[0] + pTC_h[3] + pTC_h[6] if not pd.isna(pTC_h).any() else 0
        tcD = pTC_h[1] + pTC_h[4] + pTC_h[7] if not pd.isna(pTC_h).any() else 0
        tcL = pTC_h[2] + pTC_h[5] + pTC_h[8] if not pd.isna(pTC_h).any() else 0
        ...
        m3 = 0.0
        if not pd.isna(pTC_g).any() and not pd.isna(cons_g).any():
        ...
        scores_h, tags_h = [], []
        for i in range(9):
            is_in_trend = opts_m5_h[i][-1] in n16
            is_hk_cliff = (h_hk[i] < 20 and h_hk[i]/h_365[i] <= 0.6) if h_365[i] > 0 else False
            is_poison = ( (pTC_h[i]/cons_h[i] - 1) <= -0.08 and is_in_trend ) if cons_h[i] > 0 and not pd.isna(pTC_h[i]) else False
            s, t = calculate_score(pTC_h[i], cons_h[i], math_h[i], is_in_trend, is_hk_cliff, is_poison)
            scores_h.append(s); tags_h.append(t)
            
        best_h_idx = np.argmax(scores_h) if len(scores_h) > 0 else 0
        best_h_name = opts_m5_h[best_h_idx]
        buff_goals = []
        ...
        for i in range(8):
            ...
            is_hk_cliff = (g_hk[i] / g_365[i] <= 0.5) if g_365[i] > 0 else False
            is_poison = ( (pTC_g[i]/cons_g[i] - 1) <= -0.08 and is_in_trend ) if cons_g[i] > 0 and not pd.isna(pTC_g[i]) else False
        ...
        def highlight_score(x):
            if x >= 85: return "background-color: #ffcccc; font-weight: bold; color: #dc3545;"
            elif x >= 65: return "background-color: #fff3cd; font-weight: bold; color: #fd7e14;"
            elif x < 40: return "color: #28a745;"
            return ""

        col_out1, col_out2 = st.columns(2)
        with col_out1:
            st.markdown("#### ⚽ 进球数评分切片")
            df_g = pd.DataFrame({"选项": opts_m5_g, "理论值": [f"{x*100:.1f}%" if not pd.isna(x) else "0.0%" for x in math_g], "实盘值": [f"{x*100:.1f}%" if not pd.isna(x) else "0.0%" for x in pTC_g], "危险指数": scores_g, "全息定性": tags_g}).sort_values(by="危险指数", ascending=False)
            try:
                styled_df_g = df_g.style.map(highlight_score, subset=["危险指数"])
            except AttributeError:
                styled_df_g = df_g.style.applymap(highlight_score, subset=["危险指数"])
            st.dataframe(styled_df_g, hide_index=True)

        with col_out2:
            st.markdown("#### 🔵 半全场评分切片")
            df_h = pd.DataFrame({"选项": opts_m5_h, "理论值": [f"{x*100:.1f}%" if not pd.isna(x) else "0.0%" for x in math_h], "实盘值": [f"{x*100:.1f}%" if not pd.isna(x) else "0.0%" for x in pTC_h], "危险指数": scores_h, "全息定性": tags_h}).sort_values(by="危险指数", ascending=False)
            try:
                styled_df_h = df_h.style.map(highlight_score, subset=["危险指数"])
            except AttributeError:
                styled_df_h = df_h.style.applymap(highlight_score, subset=["危险指数"])
            st.dataframe(styled_df_h, hide_index=True)

        best_g_name = df_g.iloc[0]["选项"]
        best_g_score = df_g.iloc[0]["危险指数"]
        best_h_name_res = df_h.iloc[0]["选项"]
        best_h_score_res = df_h.iloc[0]["危险指数"]
        
        st.markdown("---")
        st.markdown("### 🎯 V15.0 跨域物理化学收敛定胆")
        if best_h_score_res >= 65 and best_g_score >= 65:
            st.success(f"**✅ 高度收敛形成：** 两套模块底层逻辑产生物理化学反应。半全场主攻【{best_h_name_res}】，进球数锁定【{best_g_name}】。建议直接查阅对应比分进行波胆封锁！")
        else:
