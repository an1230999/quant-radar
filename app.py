...
# ================= 2. 核心数学引擎 (彻底抛弃Pandas切片，使用Numpy矩阵直算) =================
def calc_pure_prob(odds_series):
    # 直接提取为 Numpy 纯粹数组，无视任何格式干扰
    safe_odds = pd.to_numeric(odds_series, errors='coerce').values
    if pd.isna(safe_odds).any() or (safe_odds == 0).any():
        return np.full(len(odds_series), np.nan)
    raw_prob = 1 / safe_odds
    sum_prob = raw_prob.sum()
    return np.round(raw_prob / sum_prob, 4)
...
def generate_goal_radars(df, z2, z3, v_limit):
    ...
    df['主力狙击 (L列)'] = np.where(pd.isna(df['加速度(V-Delta)']), "➖ 无T-60不计算",
                          ...
                          np.where(df['加速度(V-Delta)'] <= -v_limit, "🩸 极极速撤离", "⚪ 匀速平稳")))
    return df
...
if active_module == "⚔️ 模块一：欧亚大盘体系 (包揽标盘让盘的浅中深)":
    ...
    def render_main_handicap_ui(water_level):
        ...
        if st.button(f"执行 {water_level} 单项研判与对冲推演", type="primary", key=f"btn_{water_level}"):
            # 使用 Numpy concatenate 直接强行拼接阵列，完全免疫Pandas索引报错
            biao_c = calc_pure_prob(edited_df['初盘'].iloc[0:3])
            biao_d = calc_pure_prob(edited_df['临场'].iloc[0:3])
            rang_c = calc_pure_prob(edited_df['初盘'].iloc[3:6])
            rang_d = calc_pure_prob(edited_df['临场'].iloc[3:6])
            
            edited_df['初概率'] = np.concatenate([biao_c, rang_c])
            edited_df['临概率'] = np.concatenate([biao_d, rang_d])
            ...
            np.where(
                (edited_df['动量(Delta)'] <= -0.015) & (edited_df['净抽水偏离'] >= 0), "🧊 真实抛弃 (时空双杀，规避)",
            np.where(
...
# ================= 5. 模块二：进球数风控 (修复防弹级数据灌入) =================
    ...
    def render_goals_ui(water_level):
        ...
        if st.button(f"执行 {water_level} 深度风控扫描", type="primary", key=f"btn_g_{water_level}"):
            ...
            # 使用 Numpy concatenate 直接强行拼接阵列
            prob_c_7 = calc_pure_prob(df['初盘(C)'].iloc[0:8])
            prob_j_7 = calc_pure_prob(df['T-60(J)'].iloc[0:8])
            prob_d_7 = calc_pure_prob(df['临场(D)'].iloc[0:8])
            
            prob_c_ou = calc_pure_prob(df['初盘(C)'].iloc[8:10])
            prob_j_ou = calc_pure_prob(df['T-60(J)'].iloc[8:10])
            prob_d_ou = calc_pure_prob(df['临场(D)'].iloc[8:10])
            
            prob_c_all = np.concatenate([prob_c_7, prob_c_ou])
            prob_j_all = np.concatenate([prob_j_7, prob_j_ou])
            prob_d_all = np.concatenate([prob_d_7, prob_d_ou])
            
            df['总动量(Delta)'] = np.round(prob_d_all - prob_c_all, 4)
            df['期望值(EV)'] = np.round(prob_c_all * df['临场(D)'].values - 1, 4)
            df['加速度(V-Delta)'] = np.round(prob_d_all - prob_j_all, 4)
            ...
            if not np.isnan(prob_c_7).all():
                # 安全提取 Numpy 数组中的元素进行加和
                even_prob = round(float(prob_c_7[[0,2,4,6]].sum()), 4)
                odd_prob = round(float(prob_c_7[[1,3,5,7]].sum()), 4)
                ...
                static_min_odds_index = df['初盘(C)'].iloc[0:8].idxmin()
