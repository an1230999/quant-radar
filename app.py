            ...
            for i in range(6):
                ...
                if is_lie:
                    ...
                    if flow >= 0.0250:
                        ...
                        scripts.append("存在微弱的受力偏移，建议结合模块一大盘轨迹研判。")
                    else:
...
# ==============================================================================
    ...
    st.info(f"📥 **大盘底座环境穿透成功** | 继承源: {m2_source_wl} | 预期大小球(xG): {mx_tg_val:.2f} | 实际让球数K: {int(mx_k_val)} | 让球赔率: [{m1_let_w:.2f}, {m1_let_d:.2f}, {m1_let_l:.2f}]")
    ...
    # 状态持久化核心一：比分输入表格
    editor_key = f"m2_editor_df_{current_match}_{m2_source_wl}"
    ...
    # 获取用户编辑后的DF并立刻存回 session_state 防止切换页面丢失
    st.session_state[editor_key] = st.data_editor(
        st.session_state[editor_key], 
        num_rows="dynamic", 
        use_container_width=True,
        key=f"editor_widget_{editor_key}"
    )
    edited_scores_df = st.session_state[editor_key]
    ...
    if st.session_state[calc_key_m2]:
        try:
            ...
            st.markdown("---")
            st.markdown("### 🧮 看板二：【总球数期权池】与【细分行权比分】对账单")
