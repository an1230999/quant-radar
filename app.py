...
# ================= 3. 防重置输入组件 (解决第一次输入消失的核心) =================
def render_number_input(label, store_key, format="%.4f", step=0.0010):
    """彻底剥离 value 属性，让组件自主管理内部状态，防止冲突重置"""
    wid_key = f"wid_{store_key}"
    # 初始化状态
    if wid_key not in st.session_state:
        st.session_state[wid_key] = st.session_state[store_key]
    
    # 渲染输入框，绝不传递 value= 参数
    st.number_input(label, format=format, step=step, key=wid_key)
    
    # 同步到持久化存储
    st.session_state[store_key] = st.session_state[wid_key]
    return st.session_state[store_key]

def render_data_editor(store_key):
    """安全渲染数据表，防止重加载失去焦点"""
    wid_key = f"wid_{store_key}"
    edited_df = st.data_editor(st.session_state[store_key], hide_index=True, num_rows="fixed", use_container_width=True, key=wid_key)
...
def init_session_state():
    if "app_v4_initialized" not in st.session_state:
        init_data_1 = [
        ...
        st.session_state["app_v4_initialized"] = True
...
def get_water_thresholds(mod, match, water_level):
    with st.expander(f"⚙️ {water_level} 专属风控阈值微调 (点击展开)"):
        ...
        with cols[0]: z2 = render_number_input("Z2 (红线)", f"{k_pre}z2{k_suf}")
        with cols[1]: z3 = render_number_input("Z3 (显著)", f"{k_pre}z3{k_suf}")
        with cols[2]: z4 = render_number_input("Z4 (警戒)", f"{k_pre}z4{k_suf}")
        with cols[3]: z5 = render_number_input("Z5 (温和)", f"{k_pre}z5{k_suf}")
        with cols[4]: z6 = render_number_input("Z6 (高赔)", f"{k_pre}z6{k_suf}", format="%.1f", step=1.0)
        with cols[5]: v_limit = render_number_input("T-60加速", f"{k_pre}v{k_suf}")
...
if active_module == "⚔️ 模块一：欧亚大盘体系 (包揽浅中深)":
    ...
    def render_main_handicap_ui(water_level, match_id):
        ...
        with col_ext1:
            h_val = render_number_input("主队亚指让球数 (决定底层映射)", hcp_key, format="%.2f", step=0.25)
            
        edited_1 = render_data_editor(df_key)
...
# ================= 8. 模块二：进球数风控 =================
    ...
    def render_goals_ui(water_level, match_id):
        ...
        col_ext1, _ = st.columns(2)
        with col_ext1:
            h_val2 = render_number_input("主队亚指让球", hcp_key, format="%.2f", step=0.25)
            
        edited_2 = render_data_editor(df_key)
...
# ================= 9. 模块三：体彩高阶工具 =================
    ...
    c1, c2, c3 = st.columns(3)
    with c1: tg = render_number_input("进球盘 (大小球)", tg_key, format="%.2f", step=0.25)
    with c2: hcp = render_number_input("让球盘 (主队亚指)", hcp_key, format="%.2f", step=0.25)
    with c3: rho = render_number_input("DC依赖系数 (ρ)", rho_key, format="%.2f", step=0.01)
    ...
    if xg_h < 0 or xg_a < 0: st.error("⚠️ 预期进球为负，请检查盘口！")
        ...
        with tab2:
            ...
            edited_3 = render_data_editor(df_key)
            
            if st.button("🚀 启动底座联动扫描", key=f"btn_{calc_flag_key}"):
                st.session_state[calc_flag_key] = True
