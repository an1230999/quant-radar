...
# ================= 5. 底座初始参数 (彻底修复：补回大盘字典！) =================
# 【核心修复】：上次手滑删掉的 M1 字典现已补回，修复模块X的 NameError 崩溃！
...
# ==============================================================================
# 解析用户当前所在页面，只有当用户“不再盯着模块二看”时，才将影子数据固化到基层！
...
for k in list(st.session_state.keys()):
    if k.startswith("m2_shadow_"):
        ...
        # 核心逻辑：如果在看别的模块，或者看的是别的比赛/水位，直接静默同步（保数据）
        # 只有当前正在该表格输入时，跳过同步（防闪退）
...
# ==============================================================================
    ...
    * 🧮 **【看板二 (总球数防假账)】**：对比球数和比分积分。**注意：如果你没有在比分台里输入对应进球数（比如0球）的比分（如0-0），看板二必然会因为没有数据而报警 `🚨 总池空壳抽水`。** 只有当对应进球数下的主要比分都输全了，这里的报警才是真正的庄家抽水铁证！
    """)
    st.markdown("---")

    # 注意：为了触发影子保护，我们必须给radio加上固定的key
    ...
    # =========================================================================
    # 🔥 核心修复点：将表格输入源与修改输出源绝对解耦，在模块内运行期保持静默，100%防闪退！
    # =========================================================================
    ...
    # 直接渲染数据编辑器，绝不在此覆写 st.session_state[base_df_key]，从物理上切断死循环！
