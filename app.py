...
if active_module == "🚀 模块十：斯巴达基本面AI预测舱 (M10)":
    ...
    with c10_2:
        st.info("💡 当前模式：【手工降维录入模式】")

    st.markdown("---")
    st.markdown("### 📥 第二步：基本面数据降维转化器")
    st.caption("填入手机App上两队最近的【总进球】和【总失球】整数，引擎会自动将其微积分转化为14大差值特征。")

    def create_m10_input_grid(period):
        st.markdown(f"#### 🏟️ 最近 {period} 场战绩统计")
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            h_h_g = safe_number_input(f"主队主场 进球", f"m10_h_h_g_{period}_{current_match}", 10.0, format="%.0f", step=1.0)
            h_h_c = safe_number_input(f"主队主场 失球", f"m10_h_h_c_{period}_{current_match}", 5.0, format="%.0f", step=1.0)
        with c2:
            h_a_g = safe_number_input(f"主队综合 进球", f"m10_h_a_g_{period}_{current_match}", 8.0, format="%.0f", step=1.0)
            h_a_c = safe_number_input(f"主队综合 失球", f"m10_h_a_c_{period}_{current_match}", 6.0, format="%.0f", step=1.0)
        with c3:
            a_a_g = safe_number_input(f"客队客场 进球", f"m10_a_a_g_{period}_{current_match}", 5.0, format="%.0f", step=1.0)
            a_a_c = safe_number_input(f"客队客场 失球", f"m10_a_a_c_{period}_{current_match}", 8.0, format="%.0f", step=1.0)
        with c4:
            a_all_g = safe_number_input(f"客队综合 进球", f"m10_a_all_g_{period}_{current_match}", 7.0, format="%.0f", step=1.0)
            a_all_c = safe_number_input(f"客队综合 失球", f"m10_a_all_c_{period}_{current_match}", 10.0, format="%.0f", step=1.0)
        
        # 特征提取
        f = {}
        f['h_h_g_avg'] = h_h_g / period
        f['h_h_c_avg'] = h_h_c / period
        f['h_a_g_avg'] = h_a_g / period
        f['h_a_c_avg'] = h_a_c / period
        f['a_a_g_avg'] = a_a_g / period
        f['a_a_c_avg'] = a_a_c / period
        f['a_all_g_avg'] = a_all_g / period
        f['a_all_c_avg'] = a_all_c / period
    ...
    with t10_1: feats_10 = create_m10_input_grid(10)
    with t10_2: feats_5 = create_m10_input_grid(5)
    with t10_3: feats_15 = create_m10_input_grid(15)
    ...
    if st.button("🚀 呼叫斯巴达量化军团 (启动 Meta-Model 预测)", type="primary", use_container_width=True):
        ...
        if abs(m10_ah) > 1.0:
            ...
            st.success("✅ 浅盘验证通过。14大时空特征降维提取完毕，正在唤醒内存中的千人专家投票网...")
