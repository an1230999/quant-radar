...
if active_module == "🪐 模块八：M8 欧亚双轨全息对撞舱 (1X2+AH)":
    ...
    if st.button("🚀 启动全白盒欧亚多维测谎", type="primary", use_container_width=True):
        ...
        try:
            ...
            # 🚀 核心修复：亚指盘口位移方向计算 (直接相减，<0为升盘，>0为退盘)
            ...
            def describe_line_tag(shift):
                if shift < 0: return f"🔺 强硬升盘 ({shift:+.2f})"  # 数值变小=升盘=门槛提高
                if shift > 0: return f"🔻 退盘诱导 ({shift:+.2f})"  # 数值变大=退盘=门槛降低
                return "➖ 盘口未动"
            ...
            # 🚀 核心修复：逻辑方向修正 (line_shift < 0 为升盘设防)
            ...
            # Scenario 3: Dark Water (Hidden Support)
                ...
                verdict_box.info("### 🛡️ 【暗度陈仓 / 亚盘暗水筑墙】\n\n"
                                 ...
                                 "**【逻辑推演过程】**：标盘故意维持高赔率装死，驱赶普通球迷的筹码；但最敏锐的亚盘防线却承受不住聪明钱的冲击，被迫强行升盘筑墙或暴跌水位进行被动防御。\n\n"
    ...
    def render_module_x_ui(wl, match_id):
        ...
        if st.session_state[calc_key]:
            ...
            if intent == "信息优势局 (机构确知)":
                ...
                st.info(f"⚠️ 由于当前处于【{intent}】，6项联合诊断被降级。资金流向反映的是市场情绪而非机构内幕意图，建议放弃本场或反向跟散户操作。")
